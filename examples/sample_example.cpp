...
int sample_f32_kernel_example() {
    ...
    try {
        ...
        auto param_tensor = tensor_factory.uniform_tensor(
                {ROWS, COLS}, data_type_t::f32, 0.5);
        auto input_tensor = tensor_factory.uniform_tensor(
                {ROWS, COLS}, data_type_t::f32, 0.5);
        auto output_tensor
                = tensor_factory.zero_tensor({ROWS, COLS}, data_type_t::f32);
        ...
        if (sample_operator.is_bad_object()) {
            log_error("operator ", sample_operator.get_name(),
                    " creation failed");
            return NOT_OK;
        }

        sample_operator.set_input("sample_input", input_tensor)
...
int sample_bf16_kernel_example() {
    ...
    try {
        tensor_factory_t tensor_factory;
        auto param_tensor = tensor_factory.uniform_tensor(
                {ROWS, COLS}, data_type_t::bf16, 0.5);
        auto input_tensor = tensor_factory.uniform_tensor(
                {ROWS, COLS}, data_type_t::bf16, 0.5);
        auto output_tensor
                = tensor_factory.zero_tensor({ROWS, COLS}, data_type_t::bf16);
        auto relu_post_op = post_op_t {post_op_type_t::relu};
        ...
        cached_sample_operator.set_input("sample_input", input_tensor)
                .set_output("sample_output", output_tensor)
                .execute();
