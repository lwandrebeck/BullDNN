...
int run_lowoha_layer_norm_fp32_example() {
    try {
        ...
        // Execute
        status_t status = normalization_direct(input.data(), output.data(),
...
int run_lowoha_layer_norm_3d_fp32_example() {
    try {
        ...
        status_t status = normalization_direct(input.data(), output.data(),
                gamma.data(), beta.data(), nullptr, nullptr,
                /*residual=*/nullptr, params);
...
int run_lowoha_batch_norm_fp32_example() {
    try {
        ...
        status_t status = normalization_direct(input.data(), output.data(),
                gamma.data(), beta.data(), running_mean.data(),
                running_var.data(),
                /*residual=*/nullptr, params);
...
int run_lowoha_rms_norm_fp32_example() {
    try {
        ...
        status_t status = normalization_direct(input.data(), output.data(),
                gamma.data(), /*beta=*/nullptr, nullptr, nullptr,
                /*residual=*/nullptr, params);
