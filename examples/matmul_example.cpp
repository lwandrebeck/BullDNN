...
int matmul_relu_f32_kernel_example() {
    ...
    try {
        ...
        auto weights = tensor_factory.uniform_tensor(
                {MATMUL_K, MATMUL_N}, data_type_t::f32, 1.0, "weights");
...
int matmul_relu_bf16_kernel_example() {
    ...
    try {
        ...
        tensor_factory_t tensor_factory;
        auto weights = tensor_factory.uniform_tensor(
                {MATMUL_K, MATMUL_N}, data_type_t::bf16, 1.0, "weights");

        auto bias = tensor_factory.uniform_tensor(
                {1, MATMUL_N}, data_type_t::bf16, 3.0, "bias");
...
int matmul_silu_add_int8_kernel_example() {
    ...
    try {
        status_t status;
        tensor_factory_t tensor_factory;
        auto wei_scales = tensor_factory.uniform_tensor(
                {1, MATMUL_N}, data_type_t::f32, 0.25, "scale tensor");

        auto weights = tensor_factory.uniform_tensor({MATMUL_K, MATMUL_N},
                data_type_t::s8, 1.0, "weights", wei_scales);

        auto bias = tensor_factory.uniform_tensor(
                {1, MATMUL_N}, data_type_t::f32, 5.0, "bias");
        ...
        auto src_scale = tensor_factory.uniform_tensor(
                {1, 1}, data_type_t::f32, 0.25, "src_scale_tensor");
        ...
        auto input_tensor = tensor_factory.uniform_tensor({MATMUL_M, MATMUL_K},
                data_type_t::s8, 1.0, "matmul_input", src_scale,
                src_zero_points);
        auto binary_tensor = tensor_factory.uniform_tensor(
                {MATMUL_M, MATMUL_N}, data_type_t::s8, 1.0, "binary_add");
        auto dst_scale_tensor = tensor_factory.uniform_tensor(
                {1, 1}, data_type_t::f32, 1.0 / 3.0, "dst_scale_output");
        auto output_tensor = tensor_factory.zero_tensor({MATMUL_M, MATMUL_N},
                data_type_t::f32, "matmul_output", dst_scale_tensor);

        status = matmul_operator.set_input("matmul_input", input_tensor)
                         .set_output("matmul_output", output_tensor)
                         .set_input(matmul_context.get_post_op(1)
...
int matmul_mul_silu_mul_f32_kernel_example() {
    testlog_info(
            "**matmul binary_mul + silu + binary_mul operator f32 kernel "
            "example.");
    ...
    try {
        ...
        auto bias = tensor_factory.uniform_tensor(
                {1, MATMUL_N}, data_type_t::f32, 3.0, "bias");
        ...
        auto mul_tensor = tensor_factory.uniform_tensor(
                {MATMUL_N}, data_type_t::f32, 2.0, "binary_mul_0");

        auto mul_tensor_2 = tensor_factory.uniform_tensor(
                {MATMUL_M, MATMUL_N}, data_type_t::bf16, 3.0, "binary_mul_1");

        auto output_tensor = tensor_factory.zero_tensor(
                {MATMUL_M, MATMUL_N}, data_type_t::f32, "matmul_output");

        status = matmul_operator.set_input("matmul_input", input_tensor)
                         .set_input(matmul_context.get_post_op(0)
                                            .binary_mul_params.tensor_name,
                                 mul_tensor)
                         .set_input(matmul_context.get_post_op(2)
                                            .binary_mul_params.tensor_name,
                                 mul_tensor_2)
                         .set_output("matmul_output", output_tensor)
                         .execute();
...
int matmul_silu_mul_bf16_kernel_example() {
    ...
    try {
        ...
        auto mul_tensor = tensor_factory.uniform_tensor(
                {MATMUL_M, MATMUL_N}, data_type_t::bf16, 2.0, "binary_mul_0");

        auto output_tensor = tensor_factory.zero_tensor(
                {MATMUL_M, MATMUL_N}, data_type_t::f32);
...
int matmul_strided_f32_kernel_example() {
    ...
    try {
        status_t status;
        tensor_factory_t tensor_factory;
        auto strided_weights = tensor_factory.uniform_dist_strided_tensor(
                {MATMUL_K, MATMUL_N}, {MATMUL_K, MATMUL_N + 10},
                data_type_t::f32, 1.0, "weights");
...
int matmul_broadcast_example() {
    ...
    try {
        ...
        tensor_factory_t tensor_factory;
        auto weights
                = tensor_factory.broadcast_uniform_tensor({MATMUL_K, MATMUL_N},
                        {0, 1}, data_type_t::f32, 1.0, "broadcasted_weight");
...
int matmul_woq_bf16_kernel_example() {
    ...
    try {
        ...
        auto wei_scales = tensor_factory.uniform_tensor(
                {1, MATMUL_N}, data_type_t::f32, 0.25, "scale tensor");
        auto weights = tensor_factory.uniform_tensor({MATMUL_K, MATMUL_N},
                data_type_t::s4, 1.0, "weights", wei_scales);
