...
int sdpa_direct_example() {
    ...
    for (size_t i = 0; i < elems; ++i) {
        const float x
                = 0.01f * static_cast<float>(static_cast<int>(i % 17) - 8);
        q[i] = x;
        k[i] = x * 1.03f;
        v[i] = x * 0.97f;
    }
    ...
    if (sdpa_direct(q.data(), k.data(), v.data(), nullptr, out.data(), params)
            != status_t::success) {
...
int sdpa_example() {
    try {
        ...
        auto query_tensor = tensor_factory.uniform_tensor(
                {BS, NUM_HEADS, SEQ_LEN, HEAD_DIM}, data_type_t::f32, 0.1f,
                "query");
        auto key_tensor = tensor_factory.uniform_tensor(
                {BS, NUM_HEADS, SEQ_LEN, HEAD_DIM}, data_type_t::f32, 0.1f,
                "key");
        auto value_tensor = tensor_factory.uniform_tensor(
                {BS, NUM_HEADS, SEQ_LEN, HEAD_DIM}, data_type_t::f32, 0.1f,
                "value");

        // Create output tensor
        auto output_tensor = tensor_factory.zero_tensor(
                {BS, NUM_HEADS, SEQ_LEN, HEAD_DIM}, data_type_t::f32, "output");
        ...
        sdpa_operator.set_output("sdpa_output", output_tensor).execute();
