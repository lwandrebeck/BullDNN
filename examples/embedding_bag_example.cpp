...
std::vector<int64_t> generate_random_indices(
        size_t count, int64_t min_val = 0, int64_t max_val = 99) {
...
int embedding_bag_f32_kernel_example() {
    ...
    try {
        ...
        auto table = tensor_factory.uniform_tensor(
                {EMB_ROW, EMB_DIM}, data_type_t::f32, 1.0, "table");
        ...
        auto indices_tensor = tensor_factory.non_uniform_tensor(
                {indices.size()}, data_type_t::s64, indices, "indices");

        auto offsets_tensor = tensor_factory.non_uniform_tensor(
                {EMB_BATCH_SIZE}, data_type_t::s64, offsets, "offsets");

        auto output_tensor = tensor_factory.zero_tensor(
                {EMB_BATCH_SIZE, EMB_DIM}, data_type_t::f32, "output");

        status = embedding_bag_operator.set_input("indices", indices_tensor)
...
int embedding_f32_kernel_example() {
    ...
    try {
        ...
        auto embedding_context
                = embag_context_t().set_param("table", table).create();
        ...
        auto output_tensor = tensor_factory.zero_tensor(
                {indices.size(), EMB_DIM}, data_type_t::f32, "output");
...
int embedding_bag_f16_kernel_example() {
    ...
    try {
        ...
        auto table = tensor_factory.uniform_tensor(
                {EMB_ROW, EMB_DIM}, data_type_t::f16, 1.0, "table");
        ...
        auto output_tensor = tensor_factory.zero_tensor(
                {EMB_BATCH_SIZE, EMB_DIM}, data_type_t::f16, "output");
...
int embedding_bag_s8_f16_kernel_example() {
    try {
        ...
        auto table = tensor_factory.quantized_embedding_tensor_random(
                {EMB_ROW, EMB_DIM}, data_type_t::s8, "table", true);
...
int group_embedding_bag_direct_example() {
    ...
    try {
        ...
        for (size_t i = 0; i < NUM_TABLES; ++i) {
            ...
            int64_t offset = 0;
            size_t indices_per_bag
                    = num_indices_per_table[i] / num_bags_per_table[i];
        ...
        for (size_t i = 0; i < NUM_TABLES; ++i) {
            outputs_data[i].resize(
                    num_bags_per_table[i] * embedding_dims[i], 0.0f);
        ...
        if (status == status_t::success) {
            ...
            testlog_error("<group_embedding_bag_direct>", " execution failed.");
            return NOT_OK;
        }
