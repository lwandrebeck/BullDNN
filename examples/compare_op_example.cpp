...
int compare_operator_execute(tensor_t &input1, tensor_t &input2) {
    try {
        ...
        auto compare_context
                = compare_context_t().set_tolerance(1e-07f).create();
        ...
        if (compare_operator.is_bad_object()) {
            testlog_error("operator ", compare_operator.get_name(),
                    " creation failed");
            return NOT_OK;
        }
        auto diff_tensor = tensor_factory.zero_tensor(
                {MATMUL_M, MATMUL_N}, data_type_t::f32);
        diff_tensor.set_name("diff_tensor");

        status = compare_operator.set_input("expected_tensor", input1)
        ...
        if (status == status_t::success) {
            ...
            testlog_verbose("Match Percent:", stats.match_percent, "%, ",
                    ...
                    "Relative Error:", stats.relative_error, ", ", "output[",
                    MATMUL_M / 2, ",", MATMUL_N / 2,
...
int compare_op_example() {
    ...
    try {
        ...
        // BF16 comparison
        auto input_tensor_bf16_1
                = tensor_factory.uniform_tensor({MATMUL_M, MATMUL_N},
                        data_type_t::bf16, 2.0, "compare_input_bf16_1");

        auto input_tensor_bf16_2
                = tensor_factory.uniform_tensor({MATMUL_M, MATMUL_N},
                        data_type_t::bf16, 2.0, "compare_input_bf16_2");

        testlog_info("Comparing bf16 tensors");
        compare_operator_execute(input_tensor_bf16_1, input_tensor_bf16_2);

        // FP32 comparison
        auto input_tensor_fp32_1
                = tensor_factory.uniform_tensor({MATMUL_M, MATMUL_N},
                        data_type_t::f32, 2.0, "compare_input_fp32_1");

        auto input_tensor_fp32_2
                = tensor_factory.uniform_tensor({MATMUL_M, MATMUL_N},
                        data_type_t::f32, 2.0, "compare_input_fp32_2");
...
int compare_ref_and_aocl_matmul_kernel_example() {
    ...
    try {
        ...
        auto output_tensor_ref = tensor_factory.zero_tensor(
                {MATMUL_M, MATMUL_N}, data_type_t::f32, "matmul_output");

        auto output_tensor_aocl = tensor_factory.zero_tensor(
                {MATMUL_M, MATMUL_N}, data_type_t::f32, "matmul_output");

        //Call to reference matmul kernel
        auto matmul_operator_ref
                = matmul_operator_t()
        ...
        status = matmul_operator_ref.set_input("matmul_input", input_tensor)
