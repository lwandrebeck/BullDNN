...
int run_lowoha_maxpool_fp32_test() {
    try {
        ...
        status_t status = pooling_direct(input.data(), output.data(), params);
...
int run_lowoha_avgpool_padding_modes_test() {
    try {
        ...
        status_t status = pooling_direct(
                input.data(), output_exclude.data(), params_exclude);

        if (status != status_t::success) {
            log_error(
                    "LOWOHA Average Pooling (exclude_padding): Execution "
                    "failed");
            return NOT_OK;
        }
        ...
        status = pooling_direct(
                input.data(), output_include.data(), params_include);

        if (status != status_t::success) {
            log_error(
                    "LOWOHA Average Pooling (include_padding): Execution "
                    "failed");
            return NOT_OK;
        }
