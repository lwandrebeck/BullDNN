...
REPS=${REPS:-3}
COOL=${COOL:-150}
OUT=/tmp/q6pre_ab.csv

cd "$HOME/BullDNN/build" || exit 1
make -j4 zendnnl > /tmp/q6pre_b.log 2>&1 || { echo BUILD_FAIL; exit 1; }
make install > /tmp/q6pre_i.log 2>&1 || { echo INSTALL_FAIL; exit 1; }
cd "$HOME/llama.cpp/build-vkz" || exit 1
cmake --build . --target llama-cli -j4 > /tmp/q6pre_l.log 2>&1 \
    || { echo LINK_FAIL; tail -20 /tmp/q6pre_l.log; exit 1; }
...
for rep in $(seq 1 "$REPS"); do
    for arm in stitch pre; do
        ...
        log=/tmp/q6pre_${arm}_$rep.log
        GGML_ZENDNN_IGNORE_AVX2=1 GGML_ZENDNN_IGNORE_GPU=1 \
        ZENDNNL_NATIVE_Q6K_PRESTITCH=$ilv ZENDNN_DISPATCH_STATS=1 \
