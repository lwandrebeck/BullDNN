...
qkv_strides derive_qkv_strides(
        qkv_layout_t layout, int64_t H, int64_t S, int64_t D) {
    qkv_strides r;
    r.d = 1;
...
void populate_sdpa_params(
        sdpa_params &p, const SdpaConfig &cfg, int64_t S_kv_eff) {
    p.batch = cfg.batch;
    ...
    auto qs = derive_qkv_strides(
            cfg.qkv_layout, cfg.num_heads, cfg.seq_len, cfg.head_dim);
    p.q_stride_b = qs.b;
    p.q_stride_h = qs.h;
    p.q_stride_s = qs.s;
    p.q_stride_d = qs.d;
    ...
    auto ks = derive_qkv_strides(
            cfg.qkv_layout, cfg.num_heads, S_kv_eff, cfg.head_dim);
    p.k_stride_b = ks.b;
    p.k_stride_h = ks.h;
    p.k_stride_s = ks.s;
    p.k_stride_d = ks.d;
...
int sdpa_lowoha_benchdnn(std::vector<SdpaConfig> configs,
    ...
    for (const auto &cfg : configs) {
        try {
            ...
            const int64_t S_kv_eff
                    = (cfg.kv_seq_len > 0) ? cfg.kv_seq_len : cfg.seq_len;

            tensor_factory_t tensor_factory;
            tensor_t q_tensor, k_tensor, v_tensor, o_tensor, mask_tensor;

            int ret = create_qkv_tensor(tensor_factory, cfg.batch,
                    cfg.num_heads, cfg.seq_len, cfg.head_dim, cfg.qkv_dt,
                    cfg.qkv_layout, "sdpa_query", q_tensor);
            if (ret != OK) {
            ...
            ret = create_qkv_tensor(tensor_factory, cfg.batch, cfg.num_heads,
                    S_kv_eff, cfg.head_dim, cfg.qkv_dt, cfg.qkv_layout,
                    "sdpa_key", k_tensor);
            if (ret != OK) {
            ...
            const data_type_t out_dt = (cfg.out_dt == data_type_t::none)
                    ? cfg.qkv_dt
                    : cfg.out_dt;
            ret = create_output_tensor(tensor_factory, cfg.batch, cfg.num_heads,
                    cfg.seq_len, cfg.head_dim, out_dt, cfg.qkv_layout,
                    o_tensor);
            if (ret != OK) {
            ...
            ret = create_mask_tensor(tensor_factory, cfg.batch, cfg.num_heads,
                    cfg.seq_len, S_kv_eff, cfg.mask_ndims, cfg.mask_dt,
                    mask_tensor);
            if (ret != OK) {
            ...
            const void *mask_ptr = (cfg.mask_ndims > 0)
                    ? mask_tensor.get_raw_handle_const()
                    : nullptr;
            ...
            for (int i = 0; i < cfg.warmup_iters && !skip; ++i) {
                status_t status = sdpa_direct(
                        q_ptr, k_ptr, v_ptr, mask_ptr, o_ptr, params);
            ...
            for (int i = 0; i < cfg.iters && !skip; ++i) {
                ...
                if (status != status_t::success) {
                    testlog_error(
                            "LOWOHA: sdpa_direct failed during benchmark "
                            "iterations.");
                    skip = true;
                    break;
                }
