...
qkv_layout_t strToQkvLayout(const std::string &str) {
    std::string v = str;
    std::transform(v.begin(), v.end(), v.begin(), ::tolower);
    if (v == "bhsd") { return qkv_layout_t::bhsd; }
    if (v == "bshd") { return qkv_layout_t::bshd; }
...
bool isSupportedSdpaConfig(const SdpaConfig &cfg, std::string &reason) {
    ...
    if (cfg.mask_ndims > 0) {
        ...
        if (cfg.qkv_dt == data_type_t::bf16 && cfg.mask_dt != data_type_t::f32
                && cfg.mask_dt != data_type_t::bf16) {
            reason = "mask_dt must be f32 or bf16 when qkv_dt is bf16";
            return false;
        }
        if (cfg.qkv_dt == data_type_t::f16 && cfg.mask_dt != data_type_t::f32
    ...
    while (std::getline(infile, line)) {
        ...
        try {
            ...
            cfg.out_dt = (id < (int)fields.size() && !fields[id].empty())
                    ? strToOptionalDatatype(fields[id])
                    : data_type_t::none;
            id++;
    ...
    while (std::getline(infile, line)) {
        ...
        if (fields.size() < 6) {
            commonlog_error(
                    ...
                    "[ModelName, batch, num_heads, seq_len, kv_seq_len, "
                    "head_dim] - got: ",
                    line);
            continue;
        }
        ...
        try {
            ...
            std::string reason;
            if (!isSupportedSdpaConfig(cfg, reason)) {
                commonlog_error("Skipping unsupported SDPA model config (",
                        cfg.modelName, "): ", reason);
                continue;
            }
            ...
            commonlog_error(
                    "Failed to parse model line '", line, "': ", e.what());
            continue;
        }
    }
}
...
void log_benchmark_failure(const SdpaConfig &cfg) {
    testlog_error(
            ...
            "batch=",
            cfg.batch, ", num_heads=", cfg.num_heads, ", seq_len=", cfg.seq_len,
            ", kv_seq_len=", cfg.kv_seq_len, ", head_dim=", cfg.head_dim,
            ...
            ", out_dt=", datatypeToStr(cfg.out_dt), ", scale=", cfg.scale,
            ", num_threads=", cfg.num_threads, ", iters=", cfg.iters,
...
std::vector<std::string> buildHeaders(const InputMode inputMode) {
    ...
    if (inputMode == InputMode::MODEL) { headers.push_back("Model_Name"); }
    headers.insert(headers.end(),
            {"Batch", "Num_Heads", "Seq_Len", "KV_Seq_Len", "Head_Dim",
                    "QKV_DT", "QKV_Layout", "Is_Causal", "Mask_Ndims",
                    "Mask_DT", "Out_DT", "Scale", "Num_Threads", "Iters",
                    "Warmup_Iters", "Total_time(ms)", "Avg_time(ms)"});
    return headers;
}

std::vector<std::string> buildRow(const SdpaConfig &cfg,
        const TimingStats &stat, const InputMode inputMode) {
    std::vector<std::string> row;
    if (inputMode == InputMode::MODEL) { row.push_back(cfg.modelName); }
    ...
    row.push_back(
            (cfg.num_threads == 0) ? "auto" : std::to_string(cfg.num_threads));
