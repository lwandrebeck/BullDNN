...
int bench(const std::string &in_filename, const std::string &out_filename,
    ...
    if (!isLOWOHA) {
        testlog_error(
                "Regular (non-LOWOHA) SDPA benchmark is not supported. "
                "Please use --lowoha=true (the default) to run the SDPA "
                "benchmark.");
        return NOT_OK;
    }
    ...
    int status
            = sdpa_lowoha_benchdnn(configs, sdpa_results, options, cache_size);
    if (status != OK) {
