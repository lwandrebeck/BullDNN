...
ZenArch detect_zen_arch() {
    ...
    if (!__get_cpuid(0, &eax, &ebx, &ecx, &edx)) return ZenArch::UNKNOWN;
...
const ArchConstants &get_arch_constants(ZenArch arch) {
    if (arch == ZenArch::UNKNOWN) arch = detect_zen_arch();
    switch (arch) {
...
PerfProfile parse_perf_profile(const char *str) {
    ...
    if (std::strcmp(str, "tlb") == 0) return PerfProfile::TLB;
    if (std::strcmp(str, "stalls") == 0) return PerfProfile::STALLS;
...
struct EventDef {
    ...
    const char *
            perf_name; // name matching `perf stat` output for analyze_benchmark.py
    uint32_t type;
    uint64_t config;
};
...
static constexpr int NUM_EVENTS_CACHE
        = sizeof(EVENTS_CACHE) / sizeof(EVENTS_CACHE[0]);
...
static const EventDef EVENTS_TLB[] = {
        ...
        {"L1_misses", "L1-dcache-load-misses", PERF_TYPE_HW_CACHE,
                (PERF_COUNT_HW_CACHE_L1D) | (PERF_COUNT_HW_CACHE_OP_READ << 8)
                        | (PERF_COUNT_HW_CACHE_RESULT_MISS << 16)},
        {"DTLB_L2_hit", "r0F45", PERF_TYPE_RAW,
                0x0F45}, // PMCx045 umask=0x0F (any L2 DTLB hit)
        {"DTLB_L2_miss", "rF045", PERF_TYPE_RAW,
                0xF045}, // PMCx045 umask=0xF0 (any L2 DTLB miss → page walk)
        {"ret_insn", "r00C0", PERF_TYPE_RAW,
                0x00C0}, // PMCx0C0 retired instructions
        {"cycles", "r0076", PERF_TYPE_RAW, 0x0076}, // PMCx076 unhalted cycles
};
static constexpr int NUM_EVENTS_TLB
        = sizeof(EVENTS_TLB) / sizeof(EVENTS_TLB[0]);
...
static const EventDef EVENTS_STALLS[] = {
        {"ret_insn", "r00C0", PERF_TYPE_RAW,
                0x00C0}, // PMCx0C0 retired instructions
        {"cycles", "r0076", PERF_TYPE_RAW, 0x0076}, // PMCx076 unhalted cycles
        {"fp_reg_stall", "r20AE", PERF_TYPE_RAW,
                0x20AE}, // PMCx0AE umask=0x20 FP regfile full
        {"fp_sched_stall", "r40AE", PERF_TYPE_RAW,
                0x40AE}, // PMCx0AE umask=0x40 FP scheduler full
        {"lq_stall", "r02AE", PERF_TYPE_RAW,
                0x02AE}, // PMCx0AE umask=0x02 load queue full
        {"retire_stall", "r20AF", PERF_TYPE_RAW,
                0x20AF}, // PMCx0AF umask=0x20 retire token stall
};
static constexpr int NUM_EVENTS_STALLS
        = sizeof(EVENTS_STALLS) / sizeof(EVENTS_STALLS[0]);

static const EventDef *get_events(PerfProfile p, int &count) {
    switch (p) {
        case PerfProfile::CACHE: count = NUM_EVENTS_CACHE; return EVENTS_CACHE;
        case PerfProfile::TLB: count = NUM_EVENTS_TLB; return EVENTS_TLB;
...
bool PerfCounterGroup::open() {
    ...
    if (!printed_banner) {
        fprintf(stderr,
                "[PERF] Detected %s (Family 0x%02X), profile=%s, "
    ...
    for (int i = 0; i < num_events; ++i) {
        ...
        if (fd < 0) {
            fprintf(stderr,
                    "[PERF] Warning: failed to open counter '%s' "
        ...
        counters_.push_back(
                {events[i].name, events[i].type, events[i].config, fd});
    }
...
static int64_t find_val(
        const std::vector<PerfCounterResult> &r, const char *name) {
    for (auto &e : r)
        if (e.name == name) return (e.value >= 0) ? e.value : 0;
    return 0;
}

PerfCounterDerived PerfCounterGroup::derive(
        double elapsed_sec, int num_threads) const {
    ...
    switch (profile_) {
        case PerfProfile::CACHE: {
            ...
            if (l2_total > 0) d.l2_miss_pct = 100.0 * l2_all_miss / l2_total;

            int64_t l3_access = l2_miss + pf_l3 + pf_dram;
            if (l3_access > 0) d.l3_miss_pct = 100.0 * pf_dram / l3_access;
        ...
        case PerfProfile::TLB: {
            ...
            if (cycles > 0) d.ipc = static_cast<double>(ret_insn) / cycles;
...
void PerfCounterGroup::print_header(bool tab) const {
    ...
    switch (profile_) {
        case PerfProfile::CACHE:
            printf("%sL1miss%%%sL2miss%%%sL3miss%%%sPF_L2%%%sPF_L3%%%sPF_DR%%%"
                   "sL2BW%%m",
                    sep, sep, sep, sep, sep, sep, sep);
            break;
        case PerfProfile::TLB:
            printf("%sL1miss%%%sDTLB_hit%%%sDTLB_walk%%%sIPC%sL2BW%%m", sep,
                    sep, sep, sep, sep);
            break;
        case PerfProfile::STALLS:
            printf("%sIPC%sFP_reg%%%sFP_sch%%%sLQ%%%sRetire%%", sep, sep, sep,
                    sep, sep);
            break;
    }
}

void PerfCounterGroup::print_values(
        const PerfCounterDerived &d, bool tab) const {
    const char *sep = tab ? "\t" : "  ";
    switch (profile_) {
        case PerfProfile::CACHE:
            printf("%s%5.1f%%%s%5.1f%%%s%5.1f%%%s%5.1f%%%s%5.1f%%%s%5.1f%%%s%5."
                   "1f%%",
                    sep, d.l1_miss_pct, sep, d.l2_miss_pct, sep, d.l3_miss_pct,
                    sep, d.pf_l2_pct, sep, d.pf_l3_pct, sep, d.pf_dram_pct, sep,
                    d.l2_bw_meas_pct);
            break;
        case PerfProfile::TLB:
            printf("%s%5.1f%%%s%5.2f%%%s%5.3f%%%s%5.2f%s%5.1f%%", sep,
                    d.l1_miss_pct, sep, d.dtlb_l2_hit_pct, sep,
                    d.dtlb_l2_miss_pct, sep, d.ipc, sep, d.l2_bw_meas_pct);
            break;
        case PerfProfile::STALLS:
            printf("%s%5.2f%s%5.1f%%%s%5.1f%%%s%5.1f%%%s%5.1f%%", sep, d.ipc,
                    sep, d.fp_reg_stall_pct, sep, d.fp_sched_stall_pct, sep,
                    d.lq_stall_pct, sep, d.retire_stall_pct);
            break;
    }
}
