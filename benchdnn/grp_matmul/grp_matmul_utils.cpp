...
bool parse_config(const std::string &line, GrpMatmulConfig &cfg) {
  ...
  try {
    ...
    std::string wp = next();
    if (!wp.empty()) {
      cfg.warmup = std::stoi(wp);
    }
    std::string moe_str = next();
    if (!moe_str.empty()) {
      cfg.moe_topk = std::stoi(moe_str);
    }
    std::string act_str = next();
    if (!act_str.empty()) {
      cfg.gated_act = std::stoi(act_str);
    }
    std::string ndown_str = next();
    if (!ndown_str.empty()) {
      cfg.N_down = std::stoi(ndown_str);
    }
    ...
    if (!total_str.empty()) {
      cfg.total_experts = std::stoi(total_str);
    }
  ...
  if (cfg.num_ops <= 0 || cfg.K <= 0 || cfg.N <= 0) {
    return false;
  }
