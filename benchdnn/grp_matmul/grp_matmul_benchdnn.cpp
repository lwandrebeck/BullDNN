...
static bool run_config(const GrpMatmulConfig &cfg, std::ostream &csv,
  ...
  if (cfg.dynamic_quant) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      params[i].quant_params.src_scale.dims = {cfg.M_per_op[i], wei_scale_G};
  ...
  if (fused_enabled) {
    ...
    const int64_t wei_down_scale_G =
      w4a8_fused ? static_cast<int64_t>(K_down / cfg.group_size) : 0;
    const size_t wei_down_scale_nelems =
      w4a8_fused ? static_cast<size_t>(wei_down_scale_G) *
      static_cast<size_t>(cfg.N_down) : 0;
    std::vector<AlignedBuffer> wei_down_scale_buf(w4a8_fused ? n : 0);
    if (w4a8_fused) {
      fused.down_scale.resize(n);
      for (int e = 0; e < n; ++e) {
        wei_down_scale_buf[e].alloc(wei_down_scale_nelems * sizeof(float));
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (w4a8_fused) {
        ...
        ds.dims = {wei_down_scale_G,
                   static_cast<int64_t>(cfg.N_down)
                  };
  ...
  if (moe_enabled) {
    ...
    total_flops += 2.0 * num_tokens * D_out *
                   topk;             // Stage 4: weighted reduce
  }
  double gflops_avg  = (total_flops / avg_ms) *
                       1e-6;             // mean throughput
  double gflops_peak = (total_flops / min_ms) *
                       1e-6;             // best-of-N throughput
