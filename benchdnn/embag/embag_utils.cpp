...
embag_algo_t strToEmbagalgo(const std::string &algo_str) {
    if (algo_str == "sum") { return embag_algo_t::sum; }
    if (algo_str == "mean") { return embag_algo_t::mean; }
    if (algo_str == "max") { return embag_algo_t::max; }
...
std::string embagalgoToStr(embag_algo_t algo) {
    switch (algo) {
        ...
        default: throw std::invalid_argument("Unknown embag_algo_t value");
    }
}
...
void inputParser(std::ifstream &infile, std::vector<EmbagConfig> &configs) {
    ...
    while (std::getline(infile, line)) {
        ...
        if (fields.size() < 12) {
            commonlog_error(
                    "Invalid line (expected 12 fields): [num_embeddings, "
                    "embedding_dims, num_bags, num_indices, algo, iterations, "
                    "dtype, fp16_scale_bias, padding_index, "
                    "include_last_offset, is_weights, scatter_stride, "
        ...
        try {
            ...
            if (fields[6].size() > 0) {
                ...
                if (dt.size() < 2) {
                    commonlog_warning(
                            "Less than 2 data types specified. Defaulting "
            ...
            if (!fields[7].empty()) {
                ...
                std::transform(fp16_scale_bias_flag.begin(),
                        fp16_scale_bias_flag.end(),
                        fp16_scale_bias_flag.begin(), ::tolower);
                if (fp16_scale_bias_flag == "true"
                        || fp16_scale_bias_flag == "1") {
                    cfg.fp16_scale_bias = true;
                } else if (fp16_scale_bias_flag == "false"
                        || fp16_scale_bias_flag == "0") {
                    cfg.fp16_scale_bias = false;
                } else {
                    commonlog_error(
                            "Invalid value for fp16_scale_bias. Use true/false "
                            "or 1/0.");
                    continue;
                }
            }
            ...
            if (fields[9].empty()) {
                commonlog_error(
                        "Field for include_last_offset is empty. Please "
                        "provide a value.");
                continue;
            }
            std::string offset_flag = fields[9];
            std::transform(offset_flag.begin(), offset_flag.end(),
                    offset_flag.begin(), ::tolower);
            if (offset_flag == "true" || offset_flag == "1") {
                ...
                commonlog_error(
                        "Include calue for include_last_offset. Use true/false "
                        "or 1/0.");
                continue;
            }
            std::string weights_flag = fields[10];
            std::transform(weights_flag.begin(), weights_flag.end(),
                    weights_flag.begin(), ::tolower);
            if (weights_flag == "true" || weights_flag == "1") {
                ...
                commonlog_error(
                        "Include calue for is_weights. Use true/false or 1/0.");
                continue;
            }
...
void log_benchmark_failure(const EmbagConfig &cfg) {
    testlog_error("Benchmark failed for ", cfg.num_embeddings, ", ",
            ...
            embagalgoToStr(cfg.algo), ", ", cfg.iters, ", ",
            datatypeToStr(cfg.dt[0]), ", ", datatypeToStr(cfg.dt[1]), ", ",
            cfg.fp16_scale_bias, ", ", cfg.padding_index, ", ",
            cfg.include_last_offset, ", ", cfg.is_weights, ", ",
...
void print_results(
    ...
    std::vector<std::string> headers = {"Num_Embeddings", "Embedding_Dims",
            "Num_Bags", "Num_Indices", "Algo", "Iterations", "Data_type",
            "Fp16_Scale_Bias", "Padding_Index", "Include_Last_Offset",
            "Is_Weights", "Scatter_Stride", "Warmup_iters",
            "Total_time(ms) (all iters)"};
    ...
    for (const auto &result : embag_results) {
        ...
        col_widths[2] = std::max(
                col_widths[2], std::to_string(config.num_bags).size() + 2);
        col_widths[3] = std::max(
                col_widths[3], std::to_string(config.num_indices).size() + 2);
        std::string algo_str = embagalgoToStr(config.algo);
        col_widths[4] = std::max(col_widths[4], algo_str.size() + 2);
        col_widths[5] = std::max(
                col_widths[5], std::to_string(config.iters).size() + 2);
        std::string dt_str = datatypeToStr(config.dt[0]) + ":"
                + datatypeToStr(config.dt[1]);
        col_widths[6] = std::max(col_widths[6], dt_str.size() + 2);
        std::string fp16_scale_bias
        ...
        col_widths[8] = std::max(
                col_widths[8], std::to_string(config.padding_index).size() + 2);
        col_widths[9] = std::max(col_widths[9],
                std::to_string(config.include_last_offset).size() + 2);
        col_widths[10] = std::max(
                col_widths[10], std::to_string(config.is_weights).size() + 2);
        col_widths[11] = std::max(col_widths[11],
                std::to_string(config.scatter_stride).size() + 2);
        col_widths[12] = std::max(
                col_widths[12], std::to_string(config.warmup_iters).size() + 2);
        std::ostringstream total_time_ss;
        total_time_ss << std::fixed << std::setprecision(2)
                      << stat.total_time_ms;
        col_widths[13]
                = std::max(col_widths[13], total_time_ss.str().size() + 2);
#if MEASURE_INDIVIDUAL_TIMINGS
        ...
        if (!isLOWOHA) {
            ...
            double ctx_creation_percentage
                    = (stat.context_creation_ms / stat.total_time_ms) * 100;
            double op_creation_percentage
                    = (stat.operator_creation_ms / stat.total_time_ms) * 100;
            double op_execution_percentage
                    = (stat.operator_execution_ms / stat.total_time_ms) * 100;
            ...
            col_widths[15]
                    = std::max(col_widths[15], op_create_str.str().size() + 2);
            col_widths[16]
                    = std::max(col_widths[16], op_exec_str.str().size() + 2);
        }
#endif
    }
...
void log_results(
        ...
        std::ostream &outfile, const bool isLOWOHA) {
    // Write CSV header
    outfile << "Num_Embeddings, Embedding_Dims, Num_Bags, Num_Indices, Algo, "
               "Iterations, Data_type, Fp16_Scale_Bias, Padding_Index, "
               "Include_Last_Offset, Is_Weights, Scatter_Stride, Warmup_iters, "
               "Total_time(ms) (all iters)";
#if MEASURE_INDIVIDUAL_TIMINGS
    // Only add individual timing headers for non-LOWOHA mode
    if (!isLOWOHA) {
        outfile << ", Context Creation Time (ms & %), Operator Creation Time "
                   "(ms & %), Operator Execution Time (ms & %)";
    }
#endif
    outfile << std::endl;
    ...
    for (const auto &result : embag_results) {
        const auto &config = result.first;
        const auto &stat = result.second;
        outfile << config.num_embeddings << ", " << config.embedding_dims
                << ", " << config.num_bags << ", " << config.num_indices << ", "
                << embagalgoToStr(config.algo) << ", " << config.iters << ", "
                ...
                << config.include_last_offset << ", " << config.is_weights
                << ", " << config.scatter_stride << ", " << config.warmup_iters
                << ", " << stat.total_time_ms;
#if MEASURE_INDIVIDUAL_TIMINGS
        // Only add individual timing data for non-LOWOHA mode
        if (!isLOWOHA) {
            ...
            double op_execution_percentage
                    = (stat.operator_execution_ms / stat.total_time_ms) * 100;
            outfile << ", " << stat.context_creation_ms << " ("
                    << ctx_creation_percentage << " %), "
                    << stat.operator_creation_ms << " ("
                    << op_creation_percentage << " %), "
                    << stat.operator_execution_ms << " ("
