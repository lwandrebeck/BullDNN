...
int run_embag(tensor_t output_tensor, tensor_t table_tensor,
    ...
    if (!isNotWarmup) {
        ...
        if (cfg.is_weights) { embag_context.set_is_weights(cfg.is_weights); }
        if (cfg.dt[0] == data_type_t::s8 || cfg.dt[0] == data_type_t::s4
        ...
        if (embag_operator.is_bad_object()) {
            testlog_error(" operator ", embag_operator.get_name(),
                    " creation failed.");
            return NOT_OK;
        }
        ...
        if (cfg.is_weights) {
            // Execute operator
            status = embag_operator.set_input("indices", indices_tensor)
        ...
        if (status == status_t::success) {
            ...
            testlog_error("<", embag_operator.get_name(), ">",
...
int embag_benchdnn(std::vector<EmbagConfig> configs,
    ...
    for (const auto &cfg : configs) {
        try {
            ...
            tensor_t table_tensor, indices_tensor, offsets_tensor,
            ...
            for (int i = 0; i < cfg.warmup_iters; ++i) {
                int ret = run_embag(output_tensor, table_tensor, indices_tensor,
                        offsets_tensor, weights_tensor, cfg, time_stats);
                if (ret != OK) {
            ...
            for (int i = 0; i < cfg.iters; ++i) {
                ...
                int ret = run_embag(output_tensor, table_tensor, indices_tensor,
                        offsets_tensor, weights_tensor, cfg, iter_stats, true);
                if (ret != OK) {
                ...
                time_stats.context_creation_ms
                        += iter_stats.context_creation_ms;
                time_stats.operator_creation_ms
                        += iter_stats.operator_creation_ms;
                time_stats.operator_execution_ms
                        += iter_stats.operator_execution_ms;
#else
...
int bench(const std::string &in_filename, const std::string &out_filename,
    ...
    if (!isLOWOHA) {
        status = embag_benchdnn(
                embagConfig, embag_results, options, cache_size);
        if (status != OK) {
            testlog_error("Embag benchmark failed.");
            return NOT_OK;
        }
    } else {
        status = embag_lowoha_benchdnn(
                embagConfig, embag_results, options, cache_size);
        if (status != OK) {
