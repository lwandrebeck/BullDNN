        ...
        try {
            ...
            const float *weights_data = cfg.is_weights
                    ? static_cast<const float *>(
                              weights_tensor.get_raw_handle_const())
                    : nullptr;
            ...
            if (cfg.scatter_stride != -1) {
                ...
                params.dst_stride
                        = output_tensor.get_stride()[0]; // Set output stride
            }

            // Validate parameters
            if (params.num_embeddings == 0 || params.embedding_dim == 0) {
                testlog_error(
                        "LOWOHA: Invalid tensor dimensions - num_embeddings:",
            ...
            for (int i = 0; i < cfg.warmup_iters && !skip; ++i) {
                ...
                if (status != status_t::success) {
                    testlog_error(
                            "LOWOHA: Embag execution failed during warm-up.");
                    skip = true;
                    break;
                }
            }
            ...
            for (int i = 0; i < cfg.iters && !skip; ++i) {
                ...
                if (status != status_t::success) {
                    testlog_error(
                            "LOWOHA: Embag execution failed during benchmark "
                            "iterations.");
                    skip = true;
                    break;
                }
