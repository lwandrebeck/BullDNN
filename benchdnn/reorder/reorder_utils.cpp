...
reorder_algo_t strToReorderAlgo(const std::string &str) {
    ...
    if (lower == "dt") { return reorder_algo_t::DT; }
    if (lower == "native") { return reorder_algo_t::native; }
    if (lower == "reference") { return reorder_algo_t::reference; }
...
void inputParser(std::ifstream &infile, std::vector<ReorderConfig> &configs,
    ...
    while (std::getline(infile, line)) {
        ...
        if (!is_lowoha) {
            ...
            if (fields.size() < 6) {
                commonlog_error(
                        "Invalid line (expected 6 fields): [rows, cols, "
                        "iterations, dtype, "
            ...
            try {
                ...
                if (static_cast<int>(cfg.rows) <= 0
                        || static_cast<int>(cfg.cols) <= 0 || cfg.iters <= 0) {
                    commonlog_error(
                            "Invalid rows/cols/iters (must be > 0), skipping "
                            "input line");
                    continue;
                }
                ...
                if (fields[4].empty()) {
                    commonlog_warning(
                            "No kernel name specified. Defaulting to 'aocl'.");
            ...
            if (fields.size() < 10) {
                commonlog_error(
                        "Invalid LOWOHA line (expected >= 10 fields): "
                        "[batch_size, rows, cols, "
                        "iters, src_dtype, dst_dtype, algo, scale_granularity, "
                        "group_size, "
                        "dynamic_quant, num_threads (optional), warmup_iters "
                        "(optional)]");
                continue;
            }
            try {
                ...
                if (static_cast<int>(cfg.batch_size) <= 0
                        || static_cast<int>(cfg.rows) <= 0
                        || static_cast<int>(cfg.cols) <= 0 || cfg.iters <= 0) {
                    commonlog_error(
                            "Invalid batch_size/rows/cols/iters (must be > 0), "
                            "skipping input line");
                    continue;
                }
                ...
                if (group_size_val < 0) {
                    commonlog_error(
                            "Invalid group_size (must be >= 0), skipping input "
                            "line");
                    continue;
                }
                ...
                if (fields.size() >= 11) {
                    ...
                    if (num_threads_val < 0) {
                        commonlog_error(
                                "Invalid num_threads (must be >= 0), skipping "
                                "input line");
                        continue;
                    }
...
void log_benchmark_failure(const ReorderConfig &cfg, bool is_lowoha) {
    if (!is_lowoha) {
        testlog_error("Benchmark failed for ", cfg.rows, ", ", cfg.cols, ", ",
                cfg.iters, ", ", datatypeToStr(cfg.dt), ", ", cfg.kernel_name,
                ", ", cfg.isInplace);
    } else {
        testlog_error("LOWOHA Benchmark failed for batch_size=", cfg.batch_size,
                ", rows=", cfg.rows, ", cols=", cfg.cols, ", iters=", cfg.iters,
...
void print_results(
    ...
    if (!isLOWOHA) {
        headers = {"Rows", "Cols", "Iterations", "Data_type", "Kernel_Name",
                "In-place", "Warmup_iters", "Total_time(ms)", "Avg_time(ms)"};
        ...
        headers = {"Batch_size", "Rows", "Cols", "Iterations", "Src_dtype",
                "Dst_dtype", "Algo", "Scale_granularity", "Group_size",
                "Dynamic_quant", "Num_threads", "Warmup_iters",
    ...
    for (const auto &result : reorder_results) {
        ...
        if (!isLOWOHA) {
            ...
            col_widths[col] = std::max(
                    col_widths[col], std::to_string(config.rows).size() + 2);
            col++;
            col_widths[col] = std::max(
                    col_widths[col], std::to_string(config.cols).size() + 2);
            col++;
            col_widths[col] = std::max(
                    col_widths[col], std::to_string(config.iters).size() + 2);
            col++;
            col_widths[col] = std::max(
                    col_widths[col], datatypeToStr(config.dt).size() + 2);
            col++;
            col_widths[col]
            ...
            op_exec_str << std::fixed << std::setprecision(2)
                        ...
                        << " %)";
            other_str << std::fixed << std::setprecision(2) << stat.other_ms
                      << " (" << other_pct << " %)";
            col_widths[col]
                    = std::max(col_widths[col], ctx_str.str().size() + 2);
            col++;
            col_widths[col]
            ...
            col++;
            col_widths[col]
                    = std::max(col_widths[col], other_str.str().size() + 2);
#endif
        } else {
            int col = 0;
            col_widths[col] = std::max(col_widths[col],
...
void log_results(
    ...
    if (!isLOWOHA) {
        outfile << "Rows, Cols, Iterations, Data type, Kernel name, In-place, "
                   "Warmup iterations, Total time (ms), Avg time (ms)";
#if MEASURE_INDIVIDUAL_TIMINGS
        outfile << ", Context Creation Time (ms & %), Operator Creation Time "
                   "(ms & %), Operator Execution Time (ms & %), Others (ms & "
                   "%)";
#endif
        outfile << std::endl;
        ...
        for (const auto &result : reorder_results) {
            const auto &config = result.first;
            const auto &stat = result.second;
            outfile << config.rows << ", " << config.cols << ", "
                    << config.iters << ", " << datatypeToStr(config.dt) << ", "
                    << config.kernel_name << ", " << config.isInplace << ", "
                    << config.warmup_iters << ", " << stat.total_time_ms << ", "
                    << (stat.total_time_ms / config.iters);
#if MEASURE_INDIVIDUAL_TIMINGS
            double ctx_pct
                    = (stat.context_creation_ms / stat.total_time_ms) * 100;
            ...
            double op_exec_pct
                    = (stat.operator_execution_ms / stat.total_time_ms) * 100;
            double other_pct = (stat.other_ms / stat.total_time_ms) * 100;
            outfile << ", " << stat.context_creation_ms << " (" << ctx_pct
                    << " %), " << stat.operator_creation_ms << " ("
                    << op_create_pct << " %), " << stat.operator_execution_ms
                    << " (" << op_exec_pct << " %), " << stat.other_ms << " ("
                    << other_pct << " %)";
#endif
            outfile << std::endl;
        }
    } else {
        outfile << "Batch size, Rows, Cols, Iterations, Src dtype, Dst dtype, "
                   "Algo, Scale granularity, Group size, Dynamic quant, Num "
                   "threads, Warmup iterations, Total time (ms), Avg time (ms)"
                << std::endl;

        for (const auto &result : reorder_results) {
            const auto &config = result.first;
            const auto &stat = result.second;
            outfile << config.batch_size << ", " << config.rows << ", "
                    << config.cols << ", " << config.iters << ", "
                    << datatypeToStr(config.src_dtype) << ", "
                    << datatypeToStr(config.dst_dtype) << ", " << config.algo
                    << ", " << config.scale_granularity << ", "
                    << config.group_size << ", " << config.dynamic_quant << ", "
                    << config.num_threads << ", " << config.warmup_iters << ", "
                    << stat.total_time_ms << ", "
