...
tensor_t::tensor_t(tensor_t &&other_) : parent_type(std::move(other_)) {
...
tensor_t &tensor_t::set_size(tensor_t::index_vec_type size_) {
    if (status != status_t::success) { option.size = size_; }
...
tensor_t &tensor_t::set_aligned_size(tensor_t::index_vec_type aligned_size_) {
    if (status != status_t::success) { option.aligned_size = aligned_size_; }
    return (*this);
}
...
tensor_t &tensor_t::set_base_index(tensor_t::index_vec_type base_) {
    if (status != status_t::success) { option.base = base_; }
    return (*this);
}
...
tensor_t &tensor_t::set_stride(tensor_t::index_vec_type stride_) {
    if (status != status_t::success) { option.stride = stride_; }
    return (*this);
}
...
bool tensor_t::is_transposed() const {
    ...
    if (strides.size() < 2 || sizes.size() < 2) { return false; }
    ...
    bool is_row_major
            = (col_stride == 1) && (row_stride == 0 || row_stride >= col_size);
...
tensor_t &tensor_t::set_quant_scale(const tensor_t &quant_scale_) {
    ...
    if (status == status_t::success) { return (*this); }

    //check if quant is iniatilized
    if (!quant) { quant.emplace(); }
...
quant_type_t tensor_t::get_quant_type() const {
    ...
    if (!quant) {
        apilog_error(
                name, " invoked a quantization api on non-quantized tensor.");
...
uint64_t tensor_t::compute_quant_scale_offset(
...
const void *tensor_t::get_quant_scale_raw_handle_const(
        const index_vec_type &index_) const {
    if (!quant) {
        apilog_error(
                name, " invoked a quantization api on non-quantized tensor.");
...
tensor_t::index_vec_type tensor_t::get_quant_zero_size() const {
    if (quant) { return quant->zero_size; }

    return {};
}

tensor_t::index_vec_type tensor_t::get_quant_zero_stride() const {
    if (quant) { return quant->zero_stride; }

    return {};
}

tensor_t::index_vec_type tensor_t::get_quant_zero_block_size() const {
    if (quant) { return quant->zero_block_size; }

    return {};
}

uint64_t tensor_t::compute_quant_zero_offset(
        const index_vec_type &index_) const {
...
data_type_t tensor_t::get_quant_zero_data_type() const {
    if (quant) { return quant->zero_data_type; }

    return data_type_t::none;
}

const void *tensor_t::get_quant_zero_raw_handle_const() const {
    if (quant) { return (const void *)(quant->zeros->get_raw_handle()); }

    return nullptr;
}

const void *tensor_t::get_quant_zero_raw_handle_const(
        const index_vec_type &index_) const {
    if (!quant) {
        apilog_error(
                name, " invoked a quantization api on non-quantized tensor.");
    ...
    return (const void *)((uint8_t *)quant->zeros->get_raw_handle()
            + zero_offset);
}

tensor_t &tensor_t::set_data_type(data_type_t data_type_) {
    if (status != status_t::success) { option.data_type = data_type_; }
    return (*this);
}
...
tensor_t &tensor_t::set_layout(tensor_layout_t layout_) {
    if (status != status_t::success) { option.layout |= uint16_t(layout_); }
    return (*this);
}
...
tensor_t &tensor_t::set_order(std::string order_) {
    if (status != status_t::success) { option.order = order_; }
    return (*this);
}
...
tensor_t &tensor_t::set_tensor_option(const tensor_option_t &option_) {
    if (status != status_t::success) { option = option_; }
    return (*this);
}
    ...
    try {
        ...
        switch (option.data_type) {
            case data_type_t::f32: {
                ...
                const cpptype *handle
                        = static_cast<const cpptype *>(raw_handle);
            ...
            default:
                std::string message
                        = "getting element with this data type is "
                          "unimplemented";
                EXCEPTION_WITH_LOC(std::move(message));
        }
    } catch (const exception_t &ex) { EXCEPTION_WITH_LOC(ex.what()); }
}
...
void *tensor_t::get_raw_handle_unsafe(const index_vec_type &index_) const {
    ...
    if (option.is_blocked() || option.is_oblique()) {
        std::string message
                = "attempt to get indexed raw handle of a blocked or oblique "
                  "tensor.";
        EXCEPTION_WITH_LOC(message);
    }
...
const void *tensor_t::get_raw_handle_const(const index_vec_type &index_) const {
    ...
    return (const void *)((uint8_t *)storage->get_raw_handle()
            + option.base_offset + index_offset);
}

tensor_t &tensor_t::set_storage() {
    if (status != status_t::success) { allocate = true; }
    return (*this);
}
...
tensor_t &tensor_t::set_storage(const tensor_t &other_) {
    if (other_.status != status_t::success) { return *this; }
    storage = other_.storage;
    return (*this);
}
...
tensor_t &tensor_t::create() {
    ...
    float multiplier = option.data_type == data_type_t::s4
                    || option.data_type == data_type_t::u4
            ? 2.0f
            : 1.0f;
    uint64_t buffer_size = std::ceil(1.0 * option.aligned_nelem
            * size_of(option.data_type) / multiplier);
    ...
    if (allocate) {
        ...
        if (buffer_size) {
            ...
            apilog_error(
                    "tensor < ", name, " > attempt to allocate zero storage.");
            status = status_t::memory_bad_storage;
            return *this;
        }
    } else {
        if (storage->get_raw_handle() == nullptr) {
            apilog_error("tensor < ", name,
                    " > unallocated or unassigned memory storage.");
            status = status_t::memory_bad_storage;
            return *this;
        }
...
status_t tensor_t::size_sanity_check() const {
    ...
    if (option.size.empty()) { return status_t::memory_bad_size; }
...
status_t tensor_t::order_sanity_check() {
    ...
    for (int8_t i = 0; i < int8_t(option.size.size()); ++i) {
        auto count = std::count(
                option.order.cbegin(), option.order.cend(), char('a' + i));
        if (count != 1) { return status_t::memory_bad_order; }
    }
...
status_t tensor_t::stride_sanity_check_with_order() {
    ...
    if (option.stride.size() != option.aligned_size.size()) {
        apilog_error("tensor ", name,
                " bad stride size: got stride.size()=", option.stride.size(),
    ...
    for (int32_t i = dims; i > 0; i--) {
        ...
        if (stride != 0) {
            ...
            if (stride != option.aligned_nelem) {
                apilog_error("tensor ", name, " invalid stride at dimension ",
                        i, ": got stride=", stride,
...
status_t tensor_t::stride_sanity_check_without_order() {
    ...
    char axis = 'a';
    for (uint32_t i = 0; i < dim; ++i) {
        if (option.stride[i] == 0) { option.order.at(i) = axis++; }
    }

    /* if any size is 1, there is a possibility of duplicate strides */
    std::vector<uint32_t> dloc;
    for (uint32_t i = 0; i < dim; ++i) {
        if (option.aligned_size[i] == 1) { dloc.push_back(i); }
    }
    ...
    for (uint32_t i = 0; i < dim; ++i) {
        ...
        if (stride[min_index] != option.aligned_nelem) {
            apilog_error("tensor < ", name, " > invalid stride at dimension ",
                    min_index, ": got stride=", stride[min_index],
                    " expected=", option.aligned_nelem);
            return status_t::memory_bad_stride;
        }
...
status_t tensor_t::index_sanity_check(const index_vec_type &index_) const {
    ...
    for (size_t i = 0; i < option.size.size(); ++i) {
        if (index_[i] > option.size[i]) { return status_t::memory_bad_index; }
    }

    return status_t::success;
}

tensor_t::index_vec_type tensor_t::permute_axes_order(
...
status_t tensor_t::validate_meta_info() {
    ...
    if (option.nelem) {
        apilog_warning("tensor < ", name,
                " > buffer size will be recomputed for contiguous/blocked "
                "tensor.");
    ...
    if (is_order) {
        ...
        if (is_stride) {
            if (stride_sanity_check_without_order() != status_t::success) {
                apilog_error("tensor < ", name,
                        " > (aligned) size and stride mismatch.");
                return status_t::memory_bad_stride;
            }
        } else {
...
tensor_t::index_vec_type tensor_t::compute_quant_block_size(
        const tensor_t::index_vec_type &quant_size_) {
...
tensor_t::index_vec_type tensor_t::compute_quant_stride(
        const tensor_t::index_vec_type &size_) {
    ...
    for (uint32_t i = 0; i < option.stride.size(); ++i) {
        if (option.stride[i] == 0) { permuted_stride[i] = 0; }
    }
...
status_t tensor_t::validate_quant_zero() {
    ...
    if ((quant->zero_block_size).empty()) { return status_t::memory_bad_quant; }
...
status_t tensor_t::validate_quant_info() {
    ...
    if (!quant) { return status_t::success; }
    ...
    if (quant->scales != nullptr) {
        ...
        apilog_error(
                name, " quant scales not provided for a quantized tensor.");
        return status_t::memory_bad_quant;
    }
