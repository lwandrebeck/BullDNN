...
status_t dynamic_module_t::load() {
    ...
    if (module_name.empty()) return status_t::utils_bad_module_name;

    try {
        module_name = "lib" + module_name + ".so";
        module_handle = dlopen(module_name.c_str(), RTLD_NOW);
        if (!module_handle) { EXCEPTION_WITH_LOC(dlerror()); }
...
void *dynamic_module_t::get_symbol(std::string symbol_name_) {
    ...
    try {
        if (module_handle) {
            ...
            if (dlerror() != NULL) EXCEPTION_WITH_LOC(dlerror());

            if (symbol_handle) return symbol_handle;
        }
    } catch (const exception_t &ex) { EXCEPTION_WITH_LOC(ex.what()); }
    return nullptr;
}
