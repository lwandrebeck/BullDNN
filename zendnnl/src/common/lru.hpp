...
template <typename KEY_T, typename VALUE_T>
lru_node_t<KEY_T, VALUE_T>::lru_node_t(
        const key_type &key_, const value_type &value_)
...
template <typename KEY_T, typename VALUE_T>
lru_cche_t<KEY_T, VALUE_T>::lru_cche_t(size_type capacity_) : lru_cche_t {} {
    capacity = capacity_;
}

template <typename KEY_T, typename VALUE_T>
status_t lru_cche_t<KEY_T, VALUE_T>::insert(
        const key_type &key_, const value_type &value_) {
    ...
    //insert new node
    [[maybe_unused]] bool insert_status
            = cache.insert({key_, lru_node_t {key_, value_}}).second;
...
status_t lru_cche_t<KEY_T, VALUE_T>::erase(const key_type &key_) {
    ...
    if (search_itr == cache.end()) { return status_t::lru_node_not_found; }
...
template <typename KEY_T, typename VALUE_T>
typename lru_cche_t<KEY_T, VALUE_T>::value_type lru_cche_t<KEY_T, VALUE_T>::at(
        const key_type &key_) {
    const std::lock_guard<std::mutex> lru_lock(lru_mutex);
...
lru_cche_t<KEY_T, VALUE_T>::get_value(const key_type &key_) {
    ...
    auto search_itr = cache.find(key_);
    if (search_itr == cache.end()) { return optional_value_type {}; }
...
template <typename KEY_T, typename VALUE_T>
void lru_cche_t<KEY_T, VALUE_T>::set_config(
        const config_lru_cache_t &config_lru_cache_) {
