...
int calculate_optimal_m_block(const BmmConfig &config) {
   ...
   */
    int threads_per_batch
            = std::max(1, config.num_threads / config.batch_count);
    int M_block = std::max(
            1, (config.M + threads_per_batch - 1) / threads_per_batch);
    ...
    if ((config.batch_count >= 1024 && config.M <= 2048)
            ...
            || (config.batch_count > 128 && config.batch_count < 192
                    && config.M <= 512)) {
...
BmmPlan plan_bmm(const BmmConfig &config) {
    ...
    plan.use_zendnnl_parallel
            = should_use_zendnnl_parallel(config.M, config.N, config.K);
    return plan;
}
