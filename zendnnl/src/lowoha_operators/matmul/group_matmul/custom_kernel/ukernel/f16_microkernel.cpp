...
__attribute__((always_inline, target("avx512f,avx512fp16,avx512bw,avx512vl,fma")))
...
static inline void swiglu_oai_store_pair_f16(
...
__attribute__((target("avx512f,avx512fp16,avx512bw,avx512vl,fma"), noinline))
static void ukernel_impl(
    const float16_t *__restrict__ A, int lda,
    const float16_t *__restrict__ Bpacked,
  ...
  static_assert(Act == ActKind::none
                    || std::is_same<DstT, float16_t>::value,
  ...
  if (has_bias) {
    ...
    if (bias_kind == BiasKind::bf16) {
      ...
      for (int h = 0; h < NV_h; ++h) {
        ...
        __m256i b_hi = _mm256_loadu_si256(
            reinterpret_cast<const __m256i *>(bias_bf16 + h * 32 + 16));
...
f16_ukernel_fn_t select_f16_ukernel(int MR, int NV, ActKind act, DstDt dst_dt) {
  ...
  const bool is_gated_act = (act == ActKind::swiglu_oai_mul
                              || act == ActKind::silu_and_mul
                              || act == ActKind::gelu_and_mul);
  if (is_gated_act && dst_dt != DstDt::kF16) {
    return nullptr;
  }
  if (NV == 2) {
    if (act == ActKind::swiglu_oai_mul) {
      switch (MR) {
        case 1: return ukernel_impl<1, 2, ActKind::swiglu_oai_mul, float16_t>;
        case 2: return ukernel_impl<2, 2, ActKind::swiglu_oai_mul, float16_t>;
        case 3: return ukernel_impl<3, 2, ActKind::swiglu_oai_mul, float16_t>;
        case 4: return ukernel_impl<4, 2, ActKind::swiglu_oai_mul, float16_t>;
        case 5: return ukernel_impl<5, 2, ActKind::swiglu_oai_mul, float16_t>;
        case 6: return ukernel_impl<6, 2, ActKind::swiglu_oai_mul, float16_t>;
        case 7: return ukernel_impl<7, 2, ActKind::swiglu_oai_mul, float16_t>;
        case 8: return ukernel_impl<8, 2, ActKind::swiglu_oai_mul, float16_t>;
        default: return nullptr;
      }
    }
    if (act == ActKind::silu_and_mul) {
      switch (MR) {
        case 1: return ukernel_impl<1, 2, ActKind::silu_and_mul, float16_t>;
        case 2: return ukernel_impl<2, 2, ActKind::silu_and_mul, float16_t>;
        case 3: return ukernel_impl<3, 2, ActKind::silu_and_mul, float16_t>;
        case 4: return ukernel_impl<4, 2, ActKind::silu_and_mul, float16_t>;
        case 5: return ukernel_impl<5, 2, ActKind::silu_and_mul, float16_t>;
        case 6: return ukernel_impl<6, 2, ActKind::silu_and_mul, float16_t>;
        case 7: return ukernel_impl<7, 2, ActKind::silu_and_mul, float16_t>;
        case 8: return ukernel_impl<8, 2, ActKind::silu_and_mul, float16_t>;
        default: return nullptr;
      }
    }
    if (act == ActKind::gelu_and_mul) {
      switch (MR) {
        case 1: return ukernel_impl<1, 2, ActKind::gelu_and_mul, float16_t>;
        case 2: return ukernel_impl<2, 2, ActKind::gelu_and_mul, float16_t>;
        case 3: return ukernel_impl<3, 2, ActKind::gelu_and_mul, float16_t>;
        case 4: return ukernel_impl<4, 2, ActKind::gelu_and_mul, float16_t>;
        case 5: return ukernel_impl<5, 2, ActKind::gelu_and_mul, float16_t>;
        case 6: return ukernel_impl<6, 2, ActKind::gelu_and_mul, float16_t>;
        case 7: return ukernel_impl<7, 2, ActKind::gelu_and_mul, float16_t>;
        case 8: return ukernel_impl<8, 2, ActKind::gelu_and_mul, float16_t>;
        default: return nullptr;
      }
    }
  ...
  if (NV == 4) {
    if (act == ActKind::swiglu_oai_mul) {
      switch (MR) {
        case 1: return ukernel_impl<1, 4, ActKind::swiglu_oai_mul, float16_t>;
        case 2: return ukernel_impl<2, 4, ActKind::swiglu_oai_mul, float16_t>;
        case 3: return ukernel_impl<3, 4, ActKind::swiglu_oai_mul, float16_t>;
        case 4: return ukernel_impl<4, 4, ActKind::swiglu_oai_mul, float16_t>;
        case 5: return ukernel_impl<5, 4, ActKind::swiglu_oai_mul, float16_t>;
        case 6: return ukernel_impl<6, 4, ActKind::swiglu_oai_mul, float16_t>;
        default: return nullptr;
      }
    }
    if (act == ActKind::silu_and_mul) {
      switch (MR) {
        case 1: return ukernel_impl<1, 4, ActKind::silu_and_mul, float16_t>;
        case 2: return ukernel_impl<2, 4, ActKind::silu_and_mul, float16_t>;
        case 3: return ukernel_impl<3, 4, ActKind::silu_and_mul, float16_t>;
        case 4: return ukernel_impl<4, 4, ActKind::silu_and_mul, float16_t>;
        case 5: return ukernel_impl<5, 4, ActKind::silu_and_mul, float16_t>;
        case 6: return ukernel_impl<6, 4, ActKind::silu_and_mul, float16_t>;
        default: return nullptr;
      }
    }
    if (act == ActKind::gelu_and_mul) {
      switch (MR) {
        case 1: return ukernel_impl<1, 4, ActKind::gelu_and_mul, float16_t>;
        case 2: return ukernel_impl<2, 4, ActKind::gelu_and_mul, float16_t>;
        case 3: return ukernel_impl<3, 4, ActKind::gelu_and_mul, float16_t>;
        case 4: return ukernel_impl<4, 4, ActKind::gelu_and_mul, float16_t>;
        case 5: return ukernel_impl<5, 4, ActKind::gelu_and_mul, float16_t>;
        case 6: return ukernel_impl<6, 4, ActKind::gelu_and_mul, float16_t>;
        default: return nullptr;
      }
    }
    if (dst_dt == DstDt::kF32) {
  ...
  return nullptr;
#else
  (void)MR; (void)NV; (void)act; (void)dst_dt;
  return nullptr;
#endif
}
