...
enum class avg_pooling_mode_t {
    include_padding
    = 0, ///< Include padding in average calculation (SAME as TensorFlow)
    exclude_padding
    = 1 ///< Exclude padding from average calculation (VALID counting)
};
...
struct pool_params {
    ...
    avg_pooling_mode_t
            avg_mode; ///< Include or exclude padding in average calculation
