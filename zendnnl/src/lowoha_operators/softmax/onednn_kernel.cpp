...
#pragma omp parallel for
                for (uint64_t i = 0; i < total_elems; ++i) {
                    softmin_scratch_16[i]
                            = static_cast<uint16_t>(bfloat16_t::f32_to_bf16_val(
...
#pragma omp parallel for
                for (uint64_t i = 0; i < total_elems; ++i) {
                    softmin_scratch_16[i] = float16_t::f32_to_f16_val(
                            -static_cast<float>(in[i]));
                }
                effective_input = softmin_scratch_16.data();
            }
        ...
        switch (params.ndims) {
            ...
            default:
                log_error("Softmax OneDNN: Unsupported number of dimensions: ",
                        params.ndims);
                return status_t::failure;
        }
        ...
        log_info("Softmax OneDNN: ", params.ndims, "D tensor (shape=[",
                shape_str.str(), "]), softmax on axis ", softmax_axis,
        ...
        if (params.log_softmax) {
            softmax_pd = dnnl::softmax_forward::primitive_desc(eng,
                    dnnl::prop_kind::forward_inference,
                    dnnl::algorithm::softmax_log, src_md, dst_md, softmax_axis);
        } else {
            softmax_pd = dnnl::softmax_forward::primitive_desc(eng,
                    dnnl::prop_kind::forward_inference,
                    dnnl::algorithm::softmax_accurate, src_md, dst_md,
                    softmax_axis);
        }

        // Create memory objects (src points to -input scratch when softmin).
        auto src_mem = dnnl::memory(
                src_md, eng, const_cast<void *>(effective_input));
