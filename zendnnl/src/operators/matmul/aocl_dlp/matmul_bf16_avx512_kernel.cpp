...
status_t matmul_bf16_avx512_kernel_t::execute(const context_type &context_,
    ...
    int16_t *weights_raw_handle
            = (int16_t *)weight_tensor.get_raw_handle_unsafe();
    ...
    auto reorder_weights
            = (int16_t *)context_.get_aocl_dlp_reordered_weights_ptr_unsafe();
    ...
    if (output_tensor.get_data_type() == data_type_t::f32) {
        for (auto bs = 0; bs < batch_size; ++bs) {
            aocl_gemm_bf16bf16f32of32(order, trans_input, trans_weight, m, n, k,
        ...
        for (auto bs = 0; bs < batch_size; ++bs) {
            aocl_gemm_bf16bf16f32obf16(order, trans_input, trans_weight, m, n,
