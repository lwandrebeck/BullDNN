...
status_t matmul_f32_avx512_kernel_t::execute(const context_type &context_,
    ...
    auto reorder_weights
            = (float *)context_.get_aocl_dlp_reordered_weights_ptr_unsafe();
    ...
    for (auto bs = 0; bs < batch_size; bs++) {
        aocl_gemm_f32f32f32of32(order, trans_input, trans_weight, m, n, k,
                alpha, input_raw_handle + bs * offset_src, lda, input_format,
