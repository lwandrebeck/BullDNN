...
status_t matmul_f16_avx512_kernel_t::execute(const context_type &context_,
    ...
    uint16_t *input_raw_handle
            = (uint16_t *)input_tensor.get_raw_handle_unsafe();
    void *output_raw_handle = output_tensor.get_raw_handle_unsafe();
    uint16_t *weights_raw_handle
            = (uint16_t *)weight_tensor.get_raw_handle_unsafe();
    ...
    auto reorder_weights
            = (uint16_t *)context_.get_aocl_dlp_reordered_weights_ptr_unsafe();
    ...
    const uint16_t alpha
            = common::float16_t::f32_to_f16_val(context_.get_alpha());
    const uint16_t beta
            = common::float16_t::f32_to_f16_val(context_.get_beta());
    ...
    if (output_tensor.get_data_type() == data_type_t::f16) {
        for (auto bs = 0; bs < batch_size; ++bs) {
            aocl_gemm_f16f16f16of16(order, trans_input, trans_weight, m, n, k,
                    alpha, input_raw_handle + bs * offset_src, lda,
                    input_format,
        ...
        for (auto bs = 0; bs < batch_size; ++bs) {
            aocl_gemm_f16f16f16of32(order, trans_input, trans_weight, m, n, k,
                    alpha, input_raw_handle + bs * offset_src, lda,
                    input_format,
                    (is_reordered_weights && weight_dim == 2)
                            ? reorder_weights
