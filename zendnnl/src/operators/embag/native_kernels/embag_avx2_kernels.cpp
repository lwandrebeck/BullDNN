...
status_t embag_f32_avx2_kernel_t::execute(const context_type &context_,
    ...
    if (output_data_type == data_type_t::f32) {
        ...
        if (indices_data_type == data_type_t::s64) {
            ...
            embag_avx2_kernel<float, int64_t, int64_t, float>(input, weights,
            ...
            embag_avx2_kernel<float, int32_t, int32_t, float>(input, weights,
                    indices, offsets, dst, width, indsz, offsz, padidx,
                    is_weights, algo, stride, include_last_offset);
        } else {
        ...
        if (indices_data_type == data_type_t::s64) {
            ...
            embag_avx2_kernel<float, int64_t, int64_t, uint16_t>(input, weights,
                    indices, offsets, dst, width, indsz, offsz, padidx,
                    is_weights, algo, stride, include_last_offset);
        } else if (indices_data_type == data_type_t::s32) {
            int32_t *indices
            ...
            embag_avx2_kernel<float, int32_t, int32_t, uint16_t>(input, weights,
                    indices, offsets, dst, width, indsz, offsz, padidx,
                    is_weights, algo, stride, include_last_offset);
        } else {
            apilog_error("Unsupported data type for indices and offsets");
...
status_t embag_bf16_avx2_kernel_t::execute(const context_type &context_,
    ...
    if (output_data_type == data_type_t::f32) {
        ...
        if (indices_data_type == data_type_t::s64) {
            ...
            embag_avx2_kernel<uint16_t, int64_t, int64_t, float>(input, weights,
                    indices, offsets, dst, width, indsz, offsz, padidx,
                    is_weights, algo, stride, include_last_offset);
        } else if (indices_data_type == data_type_t::s32) {
            int32_t *indices
            ...
            embag_avx2_kernel<uint16_t, int32_t, int32_t, float>(input, weights,
                    indices, offsets, dst, width, indsz, offsz, padidx,
                    is_weights, algo, stride, include_last_offset);
        } else {
            apilog_error("Unsupported data type for indices and offsets");
        ...
        if (indices_data_type == data_type_t::s64) {
            ...
            embag_avx2_kernel<uint16_t, int64_t, int64_t, uint16_t>(input,
            ...
            embag_avx2_kernel<uint16_t, int32_t, int32_t, uint16_t>(input,
                    weights, indices, offsets, dst, width, indsz, offsz, padidx,
                    is_weights, algo, stride, include_last_offset);
        } else {
            apilog_error("Unsupported data type for indices and offsets");
