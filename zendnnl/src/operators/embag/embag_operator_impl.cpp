...
status_t embag_impl_t::validate() {
    ...
    if (table_data_type == data_type_t::f32
        ...
        if (output_sizes[1] != table_sizes[1]) {
            log_error(obj_name, ": size mismatch in input/output/params. ",
                    obj_name, " output_size=", output_sizes[1],
    ...
    if (get_input("offsets")) {
        ...
        [[maybe_unused]] const int32_t *offsets_data
        ...
        if (include_last_offset) {
            ...
            assert(offsets_data[batch_size - 1]
...
status_t embag_impl_t::validate_forced_kernel() {
    ...
    if (forced_kernel.empty()) { return status_t::success; }
    ...
    if (forced_kernel == "reference") {
        ...
        if (get_input("offsets")) {
            ...
            if (offsets_data_type != data_type_t::s64
                    && offsets_data_type != data_type_t::s32) {
                apilog_error(
                        obj_name, ": offsets datatype must be int64 or int32");
                return status_t::failure;
            }
            ...
            if (include_last_offset) {
                assert(offsets_data[batch_size]
                                <= static_cast<int32_t>(num_indices)
                        && "offsets[batch_size] must be <= indices_sizes");
            } else {
                assert(offsets_data[batch_size - 1]
...
std::string embag_impl_t::op_execute_info() {
    ...
    if (offsets) { ss << offsets.value().tensor_info() << ","; }
...
status_t embag_impl_t::kernel_factory() {
    LOG_DEBUG_INFO(
            "<", get_name(), "> Executing kernel_factory embag_operator_t");
    try {
