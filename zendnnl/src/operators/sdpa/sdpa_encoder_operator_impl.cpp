...
status_t sdpa_encoder_impl_t::validate() {
    ...
    if (has_mask && !mask) {
        apilog_error(
                "SDPA context.has_mask() is true but no \"mask\" tensor "
                "param was set on the context");
        return status_t::failure;
    }
    if (!has_mask && mask) {
        apilog_error(
                "SDPA \"mask\" tensor param was set but "
    ...
    if (q_size[0] != k_size[0] || q_size[0] != v_size[0] || // Batch
        ...
        apilog_error(
                "Q, K, V tensor dimensions are incompatible "
    ...
    if (output_size.size() != 4 || output_size[0] != q_size[0]
            || output_size[1] != q_size[1] || output_size[2] != q_size[2]
            || output_size[3] != q_size[3]) {
        apilog_error(
                "SDPA output tensor must be 4D and match Q's "
                "[B, H, S_q, head_dim] = [",
                q_size[0], ", ", q_size[1], ", ", q_size[2], ", ", q_size[3],
                "]; got rank=", output_size.size());
        return status_t::failure;
    }
    ...
    if (key->get_data_type() != qkv_dtype
        ...
        apilog_error(
                "SDPA Q, K, V tensors must share the same dtype; got "
                "Q=",
                static_cast<int>(qkv_dtype),
    ...
    if (output_dtype != qkv_dtype) {
        apilog_error("SDPA output tensor dtype must match Q/K/V dtype (",
                static_cast<int>(qkv_dtype),
                "); got output=", static_cast<int>(output_dtype));
        return status_t::failure;
    }
    ...
    auto check_bhsd_or_bshd
            = [](const char *tensor_name, const std::vector<uint64_t> &sizes,
                      const std::vector<uint64_t> &strides) -> status_t {
    ...
    if (mask) {
        ...
        const bool mask_dt_ok
                ...
                || (qkv_dtype == data_type_t::bf16
                        && (mask_dt == data_type_t::f32
                                || mask_dt == data_type_t::bf16))
                || (qkv_dtype == data_type_t::f16
                        && (mask_dt == data_type_t::f32
                                || mask_dt == data_type_t::f16));
        if (!mask_dt_ok) {
            apilog_error(
                    "SDPA mask tensor dtype is not supported for the given "
        ...
        if (rank != 2 && rank != 4) {
            apilog_error(
                    "Mask tensor must be 2D [S_q, S_kv] or 4D "
                    "[B|1, H|1, S_q, S_kv]");
            return status_t::failure;
        }
        ...
        // 2D and 4D layouts.
        if (mask_size[rank - 2] != q_size[2]
                || mask_size[rank - 1] != k_size[2]) {
            apilog_error("Mask inner dims must be [S_q, S_kv] = [", q_size[2],
                    ", ", k_size[2], "]; got [", mask_size[rank - 2], ", ",
                    mask_size[rank - 1], "]");
            return status_t::failure;
        }
        ...
        if (rank == 4
            ...
            apilog_error(
                    "4D mask leading dims must be 1 (broadcast) or match "
                    "Q's [B, H] = [",
                    q_size[0], ", ", q_size[1], "]; got [", mask_size[0], ", ",
                    mask_size[1], "]");
            return status_t::failure;
        }
        ...
        for (size_t d = rank; d-- > 0;) {
            ...
            if (!ok) {
                apilog_error("SDPA mask stride at dim ", d, " must be ",
                        expected, is_broadcast ? " (or 0 for broadcast)" : "",
                        "; got ", mask_strides[d]);
                return status_t::failure;
            }
...
std::string sdpa_encoder_impl_t::op_execute_info() {
    ...
    if (mask) { ss << ", Mask: " << mask.value().tensor_info(); }
