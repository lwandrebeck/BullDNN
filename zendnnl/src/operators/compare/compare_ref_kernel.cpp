...
status_t compare_ref_kernel_t::compare_ref_kernel_execute_templated(
    ...
    for (size_t i = 0; i < nelem; i++) {
        ...
        if (expected_val > 1e-8f) { sum_rel_error += diff / expected_val; }

        if (diff > max_dev) { max_dev = diff; }

        if (diff < min_dev) { min_dev = diff; }

        // Check if the difference is within the tolerance range
        if (std::fabs(diff) <= tolerance) { ++match_count; }
    }
...
status_t compare_ref_kernel_t::execute(const context_type &context_,
        tensor_map_type &input_, tensor_map_type &output_) {
    LOG_DEBUG_INFO("Executing compare operator");
    auto expec_tensor = input_.at("expected_tensor");
    auto dtype = expec_tensor.get_data_type();
    if (dtype == data_type_t::f32) {
        return this->compare_ref_kernel_execute_templated<float>(
                context_, input_, output_);
    } else {
        return this->compare_ref_kernel_execute_templated<bfloat16_t>(
                context_, input_, output_);
    }
}
...
template status_t
compare_ref_kernel_t::compare_ref_kernel_execute_templated<float>(
        const op_kernel_t<compare_context_t>::context_type &context_,
        op_kernel_t<compare_context_t>::tensor_map_type &input_,
        op_kernel_t<compare_context_t>::tensor_map_type &output_);

template status_t
compare_ref_kernel_t::compare_ref_kernel_execute_templated<bfloat16_t>(
        const op_kernel_t<compare_context_t>::context_type &context_,
        op_kernel_t<compare_context_t>::tensor_map_type &input_,
        op_kernel_t<compare_context_t>::tensor_map_type &output_);
