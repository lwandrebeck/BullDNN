...
void operator_impl_t<OP_CONTEXT_T>::set_context(const context_type &context_) {
    if (status != status_t::success) { context = context_; }
}
...
template <typename OP_CONTEXT_T>
void operator_impl_t<OP_CONTEXT_T>::set_input(
        const std::string &key_, const tensor_t &input_tensor_) {
    if (status == status_t::success) { inputs[key_] = input_tensor_; }
}

template <typename OP_CONTEXT_T>
std::optional<tensor_t> operator_impl_t<OP_CONTEXT_T>::get_input(
        const std::string &key_) const {
    if (status == status_t::success) {
        auto search_ptr = inputs.find(key_);
        if (search_ptr != inputs.end()) { return search_ptr->second; }
    }
...
template <typename OP_CONTEXT_T>
void operator_impl_t<OP_CONTEXT_T>::set_output(
        const std::string &key_, const tensor_t &output_tensor_) {
    if (status == status_t::success) { outputs[key_] = output_tensor_; }
}

template <typename OP_CONTEXT_T>
std::optional<tensor_t> operator_impl_t<OP_CONTEXT_T>::get_output(
        const std::string &key_) const {
    if (status == status_t::success) {
        auto search_ptr = outputs.find(key_);
        if (search_ptr != outputs.end()) { return search_ptr->second; }
    }
...
template <typename OP_CONTEXT_T>
void operator_impl_t<OP_CONTEXT_T>::set_forced_kernel(
        const std::string &forced_kernel_) {
    if (status == status_t::success) { forced_kernel = forced_kernel_; }
}
...
void operator_impl_t<OP_CONTEXT_T>::set_validation(bool validation_flag_) {
    if (status != status_t::success) { validation_flag = validation_flag_; }
}
...
void operator_impl_t<OP_CONTEXT_T>::create() {
    ...
    if (status == status_t::success) { return; }

    /* enable time based profiling */
    profiler_t profiler;
    bool profile_enabled = is_profile_enabled();

    if (profile_enabled) { profiler.tbp_start(); }
    ...
    if (!validation_flag) {
        apilog_warning(
                "<", obj_name, "> validation is off. this can be error-prone.");
    }
    ...
    /* print to api log */
    if (apilog_verbose_enabled()) { apilog_verbose(op_create_info()); }
...
status_t operator_impl_t<OP_CONTEXT_T>::execute() {
    ...
    try {
        ...
        if (apilog_verbose_enabled() || profilelog_verbose_enabled()) {
            ...
            if (apilog_verbose_enabled()) { apilog_verbose(execution_info); }
        }
...
template <typename OP_CONTEXT_T>
status_t operator_impl_t<OP_CONTEXT_T>::load_module(
...
template <typename OP_CONTEXT_T>
status_t operator_impl_t<OP_CONTEXT_T>::load_kernel(
        const std::string &symbol_) {
    try {
        create_kernel_handle_type create_kernel_handle
                = reinterpret_cast<create_kernel_handle_type>(
                        dynamic_module->get_symbol(symbol_));
...
status_t operator_impl_t<OP_CONTEXT_T>::validate_forced_kernel() {
    if (!forced_kernel.empty()) { return status_t::failure; }
...
std::size_t operator_impl_t<OP_CONTEXT_T>::hash() {
    if (status == status_t::success) {
        if (!hash_key) { hash_key = context.hash(); }
    }
