 ...
 */
template <typename SELF_T, typename OP_CONTEXT_T,
...
operator_t<SELF_T, OP_CONTEXT_T, OP_IMPL_T>::set_context(
    ...
    if (this->hash_key) {
        ...
        if (cached_value) {
            this->impl
                    = std::static_pointer_cast<impl_type>(cached_value.value());
        } else {
            this->hash_key = 0;
    ...
    if (!this->hash_key) {
        ...
        if (this->cache_flag) {
            if (!this->impl->is_bad_object()) {
                ...
                auto type_erased_impl
                        = std::static_pointer_cast<void>(this->impl);
                zendnnl_lru_cache().insert(hash_key, type_erased_impl);
            } else {
                apilog_error("<", this->get_name(),
                        "> unable to cache a bad object.");
            }
        }
    } else {
...
template <typename SELF_T, typename OP_CONTEXT_T, typename OP_IMPL_T>
typename operator_t<SELF_T, OP_CONTEXT_T, OP_IMPL_T>::self_type &
operator_t<SELF_T, OP_CONTEXT_T, OP_IMPL_T>::set_input(
        const std::string &key_, const tensor_t &input_tensor_) {
...
template <typename SELF_T, typename OP_CONTEXT_T, typename OP_IMPL_T>
std::optional<tensor_t> operator_t<SELF_T, OP_CONTEXT_T, OP_IMPL_T>::get_input(
...
template <typename SELF_T, typename OP_CONTEXT_T, typename OP_IMPL_T>
typename operator_t<SELF_T, OP_CONTEXT_T, OP_IMPL_T>::self_type &
operator_t<SELF_T, OP_CONTEXT_T, OP_IMPL_T>::set_output(
...
template <typename SELF_T, typename OP_CONTEXT_T, typename OP_IMPL_T>
std::optional<tensor_t> operator_t<SELF_T, OP_CONTEXT_T, OP_IMPL_T>::get_output(
        const std::string &key_) const {
...
template <typename SELF_T, typename OP_CONTEXT_T, typename OP_IMPL_T>
typename operator_t<SELF_T, OP_CONTEXT_T, OP_IMPL_T>::self_type &
operator_t<SELF_T, OP_CONTEXT_T, OP_IMPL_T>::set_forced_kernel(
...
template <typename SELF_T, typename OP_CONTEXT_T, typename OP_IMPL_T>
std::string
operator_t<SELF_T, OP_CONTEXT_T, OP_IMPL_T>::get_forced_kernel() const {
    return this->impl->get_forced_kernel();
}

template <typename SELF_T, typename OP_CONTEXT_T, typename OP_IMPL_T>
typename operator_t<SELF_T, OP_CONTEXT_T, OP_IMPL_T>::self_type &
operator_t<SELF_T, OP_CONTEXT_T, OP_IMPL_T>::set_validation(
