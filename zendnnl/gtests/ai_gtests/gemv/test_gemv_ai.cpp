...
class TestGemvAI : public ::testing::TestWithParam<MatmulParamsAI> {
    ...
    CreatedTensors create_test_tensors(const MatmulParamsAI &params,
            ...
            data_type_t output_dtype, bool create_reference = false) {
        CreatedTensors t;
        ...
        const uint64_t ldb_cols = params.trans_b ? params.k + params.ldb_pad
                                                 : params.n + params.ldb_pad;
...
protected:
    ...
    tensor_t transpose_weights(const tensor_t &src, uint64_t rows,
            uint64_t cols, data_type_t dtype) {
        auto dst = AITensorFactory::create_zero_tensor(
                {cols, rows}, dtype, "gemv_weights_T");
        if (dtype == data_type_t::bf16) {
            auto *s = static_cast<const bfloat16_t *>(
                    src.get_raw_handle_const());
    ...
    bool compare_gemv_output(const tensor_t &test, const tensor_t &ref,
            uint64_t k, data_type_t out_dt, bool is_int8_src = false) {
    ...
    status_t run_gemv(tensor_t &input, tensor_t &weights, tensor_t &bias,
        ...
        try {
            if (is_lowoha_mode_enabled()) {
                ...
                auto call_kernel = [&](bool wt_const, void *dst_buf) {
                    return matmul_direct('r', false, params.trans_b, M, N, K,
                            test_alpha, input.get_raw_handle_unsafe(), lda,
                            weights.get_raw_handle_unsafe(), ldb,
                            bias.get_raw_handle_unsafe(), test_beta, dst_buf,
                            ldc, wt_const, bp, mp);
                };
                ...
                EXPECT_TRUE(compare_gemv_output(
                        tl_out, output, params.k, out_dt, is_int8))
                        << "thread_local vs global_cache_warm mismatch: "
                        << params.test_name;
                EXPECT_TRUE(compare_gemv_output(
                        gc_cold_out, output, params.k, out_dt, is_int8))
    ...
    void run_accuracy_test(const MatmulParamsAI &params) {
        ...
        if (supported) {
            EXPECT_EQ(st, status_t::success)
                    << "GEMV kernel failed for " << params.test_name;
        } else {
        ...
        if (ref_supported && st == status_t::success) {
            tensor_t ref_weights = params.trans_b
                    ? transpose_weights(
                              tensors.weights, params.n, params.k, wt_dt)
                    : tensors.weights;
            ...
            status_t ref_st = AITestUtils::run_reference_matmul(tensors.input,
                    ref_weights, tensors.bias, tensors.reference_output,
                    params.post_op_config, ref_bins);
            EXPECT_EQ(ref_st, status_t::success)
                    << "Reference failed for " << params.test_name;
            if (ref_st == status_t::success) {
                bool is_int8_in = (in_dt == data_type_t::u8
                        || in_dt == data_type_t::s8);
                bool ok = compare_gemv_output(tensors.output,
                        tensors.reference_output, params.k, out_dt, is_int8_in);
    ...
    void run_boundary_test(const MatmulParamsAI &params) {
        ...
        status_t st = run_gemv(tensors.input, tensors.weights, tensors.bias,
                tensors.output, params, tensors.binary_post_op_tensors);
        EXPECT_EQ(st, status_t::success)
                << "Boundary GEMV failed: " << params.test_name;
    }
    ...
    void run_edge_case_test(const MatmulParamsAI &params) {
        ...
        if (params.expect_success) {
            EXPECT_EQ(st, status_t::success)
                    << "Edge GEMV failed: " << params.test_name;
            if (st == status_t::success
                    && AITestUtils::is_reference_implementation_supported(in_dt,
                            wt_dt, out_dt, params.post_op_config.post_ops)) {
                std::vector<tensor_t> ref_bins;
                for (auto &p : tensors.binary_post_op_tensors) {
                    ref_bins.push_back(p.second);
                }
                ...
                if (ref_st == status_t::success) {
                    bool is_int8_in = (in_dt == data_type_t::u8
                            || in_dt == data_type_t::s8);
                    bool ok = compare_gemv_output(tensors.output,
                            tensors.reference_output, params.k, out_dt,
                            is_int8_in);
                    EXPECT_TRUE(ok) << "Edge case accuracy mismatch: "
                                    << params.test_name;
                }
            }
        }
...
TEST_P(TestGemvAI, BF16GemvTest) {
    ...
    switch (params.category) {
        ...
        default: run_accuracy_test(params); break;
    }
}

INSTANTIATE_TEST_SUITE_P(AIGemvTests, TestGemvAI,
        ::testing::ValuesIn(get_test_suite_for_mode<MatmulParamsAI,
                GemvParameterGenerator>()),
