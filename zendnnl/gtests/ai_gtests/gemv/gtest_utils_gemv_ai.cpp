...
MatmulParamsAI GemvParameterGenerator::create_gemv_param(uint64_t n, uint64_t k,
        DataTypeCombination data_types, TestCategory category,
        const PostOpConfig &post_op_config, bool trans_b, bool expect_success,
    ...
    param.test_name = prefix + "_n" + std::to_string(n) + "_k"
            + std::to_string(k) + "_" + in_s + "_" + wt_s + "_" + out_s + "_"
            + tb_s + "_" + post_op_config.config_name + "_"
        ...
        for (const auto &po : postop_cfgs) {
            for (auto n : kc_n_vals) {
                for (auto k : kc_k_vals) {
                    params.push_back(create_gemv_param(n, k, combo,
                            TestCategory::ACCURACY, po, false, true,
                            "gemv_kc"));
                }
            }
        }
    ...
    for (auto combo : gemv_bf16_combos) {
        for (auto n : tb_n_vals) {
            for (auto k : tb_k_vals) {
                params.push_back(
                        create_gemv_param(n, k, combo, TestCategory::ACCURACY,
                                no_postop, true, true, "gemv_kc_transB"));
            }
        }
    }
    // INT8 transB
    for (auto combo : gemv_int8_combos) {
        for (auto n : tb_n_vals) {
            for (auto k : tb_k_vals) {
                params.push_back(
                        create_gemv_param(n, k, combo, TestCategory::ACCURACY,
                                no_postop, true, true, "gemv_int8_kc_transB"));
            }
        }
    }
            ...
            for (auto b : beta_vals) {
                for (auto n : ab_n_vals) {
                    for (auto k : ab_k_vals) {
                        auto p = create_gemv_param(n, k, combo,
                                TestCategory::ACCURACY, no_postop, false, true,
                                "gemv_kc_ab");
                        p.alpha = a;
                        p.beta = b;
                        params.push_back(p);
...
static float random_alpha() {
    static const float pool[]
            = {1.0f, 1.0f, 1.0f, 1.0f, 0.5f, 2.0f, 0.0f, -1.0f};
        ...
        for (auto combo : combos) {
            ...
            for (const auto &po : all_postops) {
                if (!AITestUtils::is_aocl_kernel_supported(
                            in_dt, wt_dt, out_dt, po.post_ops)) {
                    continue;
                }
                for (int i = 0; i < max_per_combo; ++i) {
                    ...
                    auto p = create_gemv_param(n, k, combo,
                            TestCategory::ACCURACY, po, tb, true, prefix);
    ...
    for (auto combo : gemv_int8_combos) {
        for (const auto &po : int8_postops) {
            for (int i = 0; i < max_per_combo; ++i) {
                ...
                uint64_t k = gemv_random_dim(1, 4096);
                bool tb = (gemv_random_dim(0, 1) == 1);
                auto p = create_gemv_param(n, k, combo, TestCategory::ACCURACY,
                        po, tb, true, "gemv_int8_stress");
                p.alpha = random_alpha();
                p.beta = random_beta();
                p.ldb_pad = random_ldb_pad();
                params.push_back(std::move(p));
...
void GemvParameterGenerator::add_boundary_params(
    ...
    static const std::vector<std::pair<uint64_t, uint64_t>> boundary_nk = {
            ...
            {15, 128},
            {16, 128},
            {17, 128}, // tail<1> → tail<1> → tail<2>
            {31, 128},
            {32, 128},
            {33, 128}, // tail<2> → tail<2> → tail<3>
            {47, 128},
            {48, 128},
            {49, 128}, // tail<3> → tail<3> → tail<4>
            {63, 64},
            {64, 63},
            {65, 64}, // tail<4> → nr64<1> → nr64<1>+tail<1>
            // NP+tail combos
    ...
    for (auto combo : gemv_bf16_combos) {
        for (const auto &[n, k] : boundary_nk) {
            params.push_back(
                    create_gemv_param(n, k, combo, TestCategory::BOUNDARY,
                            no_postop, false, true, "gemv_boundary"));
        }
    }
    for (auto combo : gemv_int8_combos) {
        for (const auto &[n, k] : boundary_nk) {
            params.push_back(
                    create_gemv_param(n, k, combo, TestCategory::BOUNDARY,
                            no_postop, false, true, "gemv_int8_boundary"));
        }
    }
}
    ...
    for (auto combo : gemv_bf16_combos) {
        for (const auto &po : postop_cfgs) {
            for (const auto &[n, k] : edge_nk) {
                params.push_back(create_gemv_param(n, k, combo,
                        TestCategory::EDGE_CASE, po, false, true, "gemv_edge"));
            }
        }
    }
    for (auto combo : gemv_int8_combos) {
        for (const auto &po : postop_cfgs) {
            for (const auto &[n, k] : edge_nk) {
                params.push_back(
                        create_gemv_param(n, k, combo, TestCategory::EDGE_CASE,
                                po, false, true, "gemv_int8_edge"));
            }
        }
    }
}
...
GemvParameterGenerator::generate_comprehensive_test_suite() {
    ...
    std::cout << "[AI_GEMV] Generated " << all.size() << " GEMV test cases"
              << std::endl;
    return all;
}
...
GemvParameterGenerator::generate_minimal_test_suite() {
    ...
    for (auto combo : gemv_bf16_combos) {
        ...
        minimal.push_back(create_gemv_param(64, 128, combo,
                TestCategory::ACCURACY, no_postop, false, true, "gemv_min"));
        minimal.push_back(create_gemv_param(128, 256, combo,
                TestCategory::ACCURACY, relu_po, false, true, "gemv_min"));
        minimal.push_back(create_gemv_param(256, 512, combo,
                TestCategory::ACCURACY, no_postop, false, true, "gemv_min"));
        // Looper path
        minimal.push_back(create_gemv_param(512, 256, combo,
                TestCategory::ACCURACY, no_postop, false, true, "gemv_min"));
        minimal.push_back(create_gemv_param(1024, 64, combo,
                TestCategory::ACCURACY, relu_po, false, true, "gemv_min"));
        // transB
        minimal.push_back(create_gemv_param(128, 256, combo,
                TestCategory::ACCURACY, no_postop, true, true, "gemv_min"));
        // Boundary
        minimal.push_back(create_gemv_param(1, 1, combo, TestCategory::BOUNDARY,
                no_postop, false, true, "gemv_min"));
        minimal.push_back(create_gemv_param(256, 256, combo,
                TestCategory::BOUNDARY, no_postop, false, true, "gemv_min"));
    }
