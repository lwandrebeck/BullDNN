...
bool EmbagTestUtils::validate_embag_dimensions(uint64_t num_embeddings,
        uint64_t embedding_dim, uint64_t num_indices, uint64_t num_bags) {
    if (num_embeddings < 1 || embedding_dim < 1 || num_indices < 1) {
        return false;
    }
    // num_bags can be 0 for embedding lookup mode
    if (num_bags > num_indices) { return false; }
    return true;
}
...
data_type_t EmbagTestUtils::get_table_dtype(EmbagDataTypeCombination combo) {
    switch (combo) {
        ...
        case EmbagDataTypeCombination::F32_BF16: return data_type_t::f32;
        case EmbagDataTypeCombination::BF16_BF16:
        case EmbagDataTypeCombination::BF16_F32: return data_type_t::bf16;
        case EmbagDataTypeCombination::U4_F32:
        case EmbagDataTypeCombination::U4_BF16: return data_type_t::u4;
        case EmbagDataTypeCombination::S8_F32:
        case EmbagDataTypeCombination::S8_BF16: return data_type_t::s8;
        case EmbagDataTypeCombination::S4_F32:
        case EmbagDataTypeCombination::S4_BF16: return data_type_t::s4;
...
data_type_t EmbagTestUtils::get_output_dtype(EmbagDataTypeCombination combo) {
    switch (combo) {
        ...
        case EmbagDataTypeCombination::U4_F32:
        case EmbagDataTypeCombination::S8_F32:
        case EmbagDataTypeCombination::S4_F32: return data_type_t::f32;
        case EmbagDataTypeCombination::F32_BF16:
        case EmbagDataTypeCombination::BF16_BF16:
        case EmbagDataTypeCombination::U4_BF16:
        case EmbagDataTypeCombination::S8_BF16:
        case EmbagDataTypeCombination::S4_BF16: return data_type_t::bf16;
        default: return data_type_t::f32;
    }
}
...
data_type_t EmbagTestUtils::get_indices_dtype(EmbagIndicesType indices_type) {
    switch (indices_type) {
        case EmbagIndicesType::S64: return data_type_t::s64;
        case EmbagIndicesType::S32: return data_type_t::s32;
        default: return data_type_t::s64;
    }
}
...
bool EmbagTestUtils::is_embag_kernel_supported(
        data_type_t table_dtype, data_type_t output_dtype, embag_algo_t algo) {
...
bool EmbagTestUtils::is_embag_reference_supported(
        data_type_t table_dtype, data_type_t output_dtype, embag_algo_t algo) {
...
std::vector<uint64_t> EmbagTestUtils::get_table_dims(
...
std::vector<uint64_t> EmbagTestUtils::get_offsets_dims(
        uint64_t num_bags, bool include_last_offset) {
    return {include_last_offset ? num_bags + 1 : num_bags};
}

std::vector<uint64_t> EmbagTestUtils::get_output_dims_bag(
        uint64_t num_bags, uint64_t embedding_dim) {
    return {num_bags, embedding_dim};
}

std::vector<uint64_t> EmbagTestUtils::get_output_dims_lookup(
        uint64_t num_indices, uint64_t embedding_dim) {
    return {num_indices, embedding_dim};
}

std::vector<int64_t> EmbagTestUtils::generate_random_indices(
        uint64_t num_indices, uint64_t num_embeddings, int64_t padding_idx) {
...
std::vector<int64_t> EmbagTestUtils::generate_offsets(
        uint64_t num_bags, uint64_t num_indices, bool include_last_offset) {
    std::vector<int64_t> offsets;
    if (num_bags == 0) { return offsets; }
    ...
    if (include_last_offset) { offsets.push_back(num_indices); }

    return offsets;
}

std::vector<int64_t> EmbagTestUtils::generate_boundary_indices(
        uint64_t num_indices, uint64_t num_embeddings) {
    std::vector<int64_t> indices(num_indices);
...
bool EmbagTestUtils::compare_embag_tensors(const tensor_t &test_tensor,
    ...
    return AITestUtils::compare_sampled_tensors(
            test_tensor, ref_tensor, abs_tolerance, rel_tolerance);
}

status_t EmbagTestUtils::run_reference_embag(tensor_t &table, tensor_t &indices,
        tensor_t &offsets, tensor_t &output, const EmbagParamsAI &params) {
    try {
        ...
        if (params.use_offsets) { inputs["offsets"] = offsets; }
        ...
        if (!embag_context.check()) { return status_t::failure; }
        ...
        if (embag_operator.is_bad_object()) { return status_t::failure; }
        ...
        std::cerr << "[AI_EMBAG_REF] Exception in run_reference_embag: "
                  << e.what() << std::endl;
...
float EmbagTestUtils::get_accuracy_tolerance(data_type_t output_dtype) {
    switch (output_dtype) {
        case data_type_t::f32: return AI_EMBAG_TOLERANCE_F32;
        case data_type_t::bf16: return AI_EMBAG_TOLERANCE_BF16;
        default: return AI_EMBAG_TOLERANCE_DEFAULT;
    }
}

float EmbagTestUtils::get_relative_tolerance(data_type_t output_dtype) {
    switch (output_dtype) {
        case data_type_t::f32: return AI_EMBAG_REL_TOLERANCE_F32;
        case data_type_t::bf16: return AI_EMBAG_REL_TOLERANCE_BF16;
        default: return AI_EMBAG_REL_TOLERANCE_DEFAULT;
    }
}

float EmbagTestUtils::get_epsilon_value(data_type_t dtype) {
    switch (dtype) {
        case data_type_t::f32: return AI_EMBAG_EPSILON_F32;
        case data_type_t::bf16: return AI_EMBAG_EPSILON_BF16;
        default: return AI_EMBAG_EPSILON_DEFAULT;
    }
}
...
EmbagParameterGenerator::generate_coverage_test_suite() {
    ...
    for (auto data_combo : supported_combinations) {
        if (EmbagTestUtils::is_embag_kernel_supported(
            ...
            coverage_params.push_back(create_param(cov_embeddings, cov_dim,
                    cov_indices, cov_bags, data_combo, embag_algo_t::sum,
                    TestCategory::ACCURACY, true, true,
                    "coverage_embag_dtype"));
        }
    }

    // 2. Each algorithm type for ALL data type combinations
    std::vector<embag_algo_t> algos
            = {embag_algo_t::sum, embag_algo_t::mean, embag_algo_t::max};
    for (auto data_combo : supported_combinations) {
        for (auto algo : algos) {
            if (EmbagTestUtils::is_embag_kernel_supported(
                        EmbagTestUtils::get_table_dtype(data_combo),
    ...
    std::vector<std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, std::string>>
            shapes = {{10, 8, 4, 2, "tiny"}, {100, 64, 16, 4, "small"},
        ...
        for (auto algo : algos) {
            if (EmbagTestUtils::is_embag_kernel_supported(
                ...
                for (const auto &[num_emb, dim, indices, bags, desc] : shapes) {
                    coverage_params.push_back(create_param(num_emb, dim,
                            indices, bags, data_combo, algo,
                            TestCategory::ACCURACY, true, true,
                            "coverage_embag_shape_" + desc + "_" + algo_name));
                }
            }
        }
    }
    ...
    std::vector<std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, std::string>>
            boundary_dims
            = {{10, 8, 1, 1, "minimal"}, {10, 8, 10, 1, "single_bag"},
        ...
        for (auto algo : algos) {
            if (EmbagTestUtils::is_embag_kernel_supported(
                ...
                std::string algo_name = (algo == embag_algo_t::sum) ? "sum"
                        : (algo == embag_algo_t::mean)              ? "mean"
                                                                    : "max";
                for (const auto &[num_emb, dim, indices, bags, desc] :
                        boundary_dims) {
                    coverage_params.push_back(create_param(num_emb, dim,
                            indices, bags, data_combo, algo,
                            TestCategory::BOUNDARY, true, true,
                            "coverage_embag_boundary_" + desc + "_"
                                    + algo_name));
                }
            }
        }
    ...
    std::vector<std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, std::string>>
            edge_dims = {{10000, 8, 4, 2, "large_table_tiny_dim"},
        ...
        for (auto algo : algos) {
            if (EmbagTestUtils::is_embag_kernel_supported(
                ...
                for (const auto &[num_emb, dim, indices, bags, desc] :
                    ...
                    coverage_params.push_back(create_param(num_emb, dim,
                            indices, bags, data_combo, algo,
                            TestCategory::EDGE_CASE, true, true,
                            "coverage_embag_edge_" + desc + "_" + algo_name));
                }
            }
        }
    }
        ...
        for (auto algo : algos) {
            std::string algo_name = (algo == embag_algo_t::sum) ? "sum"
                    : (algo == embag_algo_t::mean)              ? "mean"
                                                                : "max";
            // Zero embeddings
            coverage_params.push_back(create_param(0, 64, 16, 4, data_combo,
                    algo, TestCategory::INVALID, true, false,
                    "coverage_embag_invalid_zero_emb_" + algo_name));
            // Zero dimension
            coverage_params.push_back(create_param(100, 0, 16, 4, data_combo,
                    algo, TestCategory::INVALID, true, false,
                    "coverage_embag_invalid_zero_dim_" + algo_name));
            // Zero bags
            coverage_params.push_back(create_param(100, 64, 16, 0, data_combo,
                    algo, TestCategory::INVALID, true, false,
                    "coverage_embag_invalid_zero_bags_" + algo_name));
        }
    }
    ...
    std::vector<std::tuple<uint64_t, uint64_t, uint64_t, std::string>>
            lookup_shapes = {{100, 64, 32, "small"}, {1000, 128, 64, "medium"}};
    for (auto data_combo : supported_combinations) {
        if (EmbagTestUtils::is_embag_kernel_supported(
                    EmbagTestUtils::get_table_dtype(data_combo),
                    EmbagTestUtils::get_output_dtype(data_combo),
            ...
            for (const auto &[num_emb, dim, indices, desc] : lookup_shapes) {
                coverage_params.push_back(create_param(num_emb, dim, indices, 0,
                        data_combo, embag_algo_t::none, TestCategory::EDGE_CASE,
        ...
        for (auto algo : algos) {
            if (EmbagTestUtils::is_embag_kernel_supported(
                ...
                EmbagParamsAI group_small = create_param(100, 64, 32, 8,
                        data_combo, algo, TestCategory::ACCURACY, true, true,
                        "coverage_embag_group_small_" + algo_name);
                ...
                EmbagParamsAI group_medium = create_param(1000, 128, 64, 16,
                        data_combo, algo, TestCategory::ACCURACY, true, true,
                        "coverage_embag_group_medium_" + algo_name);
    ...
    for (auto data_combo : supported_combinations) {
        for (auto algo : algos) {
            if (EmbagTestUtils::is_embag_kernel_supported(
                ...
                coverage_params.push_back(create_param(100, 64, 32, 8,
                        data_combo, algo, TestCategory::ACCURACY, true, true,
                        "coverage_embag_s32idx_" + algo_name,
        ...
        if (EmbagTestUtils::is_embag_kernel_supported(
                    ...
                    EmbagTestUtils::get_output_dtype(data_combo),
                    embag_algo_t::none)) {
            coverage_params.push_back(create_param(100, 64, 32, 0, data_combo,
                    embag_algo_t::none, TestCategory::EDGE_CASE, false, true,
                    "coverage_embag_s32idx_lookup", EmbagIndicesType::S32));
        }
    }
...
EmbagParameterGenerator::generate_minimal_test_suite() {
    ...
    minimal_params.push_back(create_param(10, 8, 4, 2,
            EmbagDataTypeCombination::F32_F32, embag_algo_t::sum,
            TestCategory::BOUNDARY, true, true, "minimal"));

    // Add minimal invalid test
    minimal_params.push_back(create_param(0, 8, 4, 2,
            EmbagDataTypeCombination::F32_F32, embag_algo_t::sum,
            TestCategory::INVALID, true, false, "minimal"));
...
EmbagParameterGenerator::generate_random_params_for_accuracy_subcategory(
        const std::string &category, EmbagDataTypeCombination data_combo,
        embag_algo_t algo, bool expect_success) {
...
void EmbagParameterGenerator::add_minimal_accuracy_params(
    ...
    for (auto data_combo : supported_combinations) {
        ...
        if (data_combo == EmbagDataTypeCombination::U4_F32) { continue; }
            ...
            for (const auto &[num_emb, emb_dim, num_idx, num_bag, desc] :
                ...
                if (EmbagTestUtils::is_embag_kernel_supported(
                    ...
                    params.push_back(create_param(num_emb, emb_dim, num_idx,
                            num_bag, data_combo, algo, TestCategory::ACCURACY,
                            true, true, "minimal"));
                }
            }
        }
        ...
        for (auto algo : algos) {
            for (const auto &[num_emb, emb_dim, num_idx, num_bag, desc] :
                ...
                if (EmbagTestUtils::is_embag_kernel_supported(
                            ...
                            algo)) {
                    params.push_back(create_param(num_emb, emb_dim, num_idx,
                            num_bag, data_combo, algo, TestCategory::BOUNDARY,
                            true, true, "boundary"));
                }
            }
        }
    ...
    for (auto data_combo : supported_combinations) {
        for (const auto &[num_emb, emb_dim, num_idx, num_bag] : edge_dims) {
            if (EmbagTestUtils::is_embag_kernel_supported(
                        ...
                        embag_algo_t::sum)) {
                params.push_back(create_param(num_emb, emb_dim, num_idx,
                        num_bag, data_combo, embag_algo_t::sum,
                        TestCategory::EDGE_CASE, true, true, "edge_case"));
            }
        }
    }
}
...
void EmbagParameterGenerator::add_invalid_params(
    ...
    for (auto data_combo : supported_combinations) {
        ...
        params.push_back(
                create_param(0, 64, 16, 4, data_combo, embag_algo_t::sum,
                        TestCategory::INVALID, true, false, "invalid"));
        params.push_back(
                create_param(100, 0, 16, 4, data_combo, embag_algo_t::sum,
                        TestCategory::INVALID, true, false, "invalid"));
        params.push_back(
                create_param(100, 64, 0, 4, data_combo, embag_algo_t::sum,
                        TestCategory::INVALID, true, false, "invalid"));
...
void EmbagParameterGenerator::generate_reference_kernel_exhaustive_params(
    ...
    std::vector<embag_algo_t> algos = {embag_algo_t::sum, embag_algo_t::mean,
            embag_algo_t::max, embag_algo_t::none};
        ...
        for (auto algo : algos) {
            for (const auto &[num_emb, emb_dim, num_idx, num_bag, desc] :
                ...
                if (EmbagTestUtils::is_embag_reference_supported(
                    ...
                    params.push_back(create_param(num_emb, emb_dim, num_idx,
                            use_offsets ? num_bag : 0, data_combo, algo,
                            TestCategory::REFERENCE_KERNEL, use_offsets, true,
                            "reference"));
                }
            }
        }
...
EmbagParamsAI EmbagParameterGenerator::create_param(uint64_t num_embeddings,
        uint64_t embedding_dim, uint64_t num_indices, uint64_t num_bags,
        EmbagDataTypeCombination data_types, embag_algo_t algo,
        TestCategory category, bool use_offsets, bool expect_success,
        const std::string &suite_name, EmbagIndicesType indices_type) {
    ...
    auto algo_to_str = [](embag_algo_t a) {
        switch (a) {
            ...
            case embag_algo_t::none: return "lookup";
            default: return "unk";
        }
    };
    ...
    auto indices_type_to_str = [](EmbagIndicesType it) {
        switch (it) {
            case EmbagIndicesType::S64: return "idx64";
            case EmbagIndicesType::S32: return "idx32";
            default: return "idxunk";
        }
    };

    std::string table_dtype_str
            = dtype_to_str(EmbagTestUtils::get_table_dtype(data_types));
    std::string output_dtype_str
            = dtype_to_str(EmbagTestUtils::get_output_dtype(data_types));
    ...
    param.test_name = suite_prefix + "emb" + std::to_string(num_embeddings)
            + "_dim" + std::to_string(embedding_dim) + "_idx"
            + std::to_string(num_indices) + "_bag" + std::to_string(num_bags)
            + "_tbl_" + table_dtype_str + "_out_" + output_dtype_str + "_"
            + indices_type_str + "_" + algo_str + "_"
