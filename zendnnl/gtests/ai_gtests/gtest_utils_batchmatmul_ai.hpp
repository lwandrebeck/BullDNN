...
public:
    ...
    static status_t run_reference_batchmatmul(tensor_t &input,
    ...
    static std::vector<uint64_t> get_input_dims(
            uint64_t batch_size, uint64_t m, uint64_t k, bool broadcast_input);
    static std::vector<uint64_t> get_weight_dims(uint64_t batch_size,
            uint64_t k, uint64_t n, bool broadcast_weights);
    static std::vector<uint64_t> get_output_dims(
...
private:
    ...
    static std::vector<
            std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, std::string>>
    ...
    static BatchMatmulParamsAI create_param(uint64_t batch_size, uint64_t m,
            uint64_t n, uint64_t k, DataTypeCombination data_types,
            TestCategory category, const PostOpConfig &post_op_config,
            bool broadcast_weights = false, bool broadcast_input = false,
