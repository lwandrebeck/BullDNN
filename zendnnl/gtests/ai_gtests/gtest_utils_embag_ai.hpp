...
public:
    ...
    static std::vector<int64_t> generate_random_indices(uint64_t num_indices,
            uint64_t num_embeddings, int64_t padding_idx = -1);
    static std::vector<int64_t> generate_offsets(uint64_t num_bags,
            uint64_t num_indices, bool include_last_offset = false);
    static std::vector<int64_t> generate_boundary_indices(
            uint64_t num_indices, uint64_t num_embeddings);

    // Tensor comparison
    static bool compare_embag_tensors(const tensor_t &test_tensor,
            const tensor_t &ref_tensor, float abs_tolerance,
            float rel_tolerance);

    // Reference implementation
    static status_t run_reference_embag(tensor_t &table, tensor_t &indices,
...
private:
    ...
    static EmbagParamsAI create_param(uint64_t num_embeddings,
            uint64_t embedding_dim, uint64_t num_indices, uint64_t num_bags,
            EmbagDataTypeCombination data_types, embag_algo_t algo,
            TestCategory category, bool use_offsets = true,
