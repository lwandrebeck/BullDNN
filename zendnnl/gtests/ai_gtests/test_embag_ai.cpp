...
class TestEmbagAI : public ::testing::TestWithParam<EmbagParamsAI> {
    ...
    CreatedTensors create_test_tensors(const EmbagParamsAI &params,
            ...
            bool use_boundary = false, bool create_reference_output = false,
        ...
        // Get dimensions
        auto table_dims = EmbagTestUtils::get_table_dims(
                params.num_embeddings, params.embedding_dim);
        auto indices_dims
                = EmbagTestUtils::get_indices_dims(params.num_indices);
        auto output_dims = params.use_offsets
                ? EmbagTestUtils::get_output_dims_bag(
                          params.num_bags, params.embedding_dim)
                : EmbagTestUtils::get_output_dims_lookup(
        ...
        if (table_dtype == data_type_t::u4 || table_dtype == data_type_t::s8
            ...
            tensors.table = AITensorFactory::create_quantized_embedding_tensor(
                    table_dims, table_dtype, table_name,
                    params.fp16_scale_bias);
        } else {
        ...
        if (use_boundary) {
            indices_data = EmbagTestUtils::generate_boundary_indices(
                    params.num_indices, params.num_embeddings);
        } else {
            indices_data = EmbagTestUtils::generate_random_indices(
                    params.num_indices, params.num_embeddings,
                    params.use_padding_idx ? params.padding_idx : -1);
        }

        data_type_t indices_dtype
                = EmbagTestUtils::get_indices_dtype(params.indices_type);
        std::vector<tensor_t::index_type> indices_size_vec(
                indices_dims.begin(), indices_dims.end());
        ...
        if (params.use_offsets) {
            auto offsets_dims = EmbagTestUtils::get_offsets_dims(
                    params.num_bags, params.include_last_offset);
            std::vector<int64_t> offsets_data
                    = EmbagTestUtils::generate_offsets(params.num_bags,
                            params.num_indices, params.include_last_offset);

            std::vector<tensor_t::index_type> offsets_size_vec(
                    offsets_dims.begin(), offsets_dims.end());
...
protected:
    ...
    void run_reference_kernel_test(const EmbagParamsAI &params) {
        ASSERT_TRUE(EmbagTestUtils::validate_embag_dimensions(
                params.num_embeddings, params.embedding_dim, params.num_indices,
                params.num_bags))
        ...
        auto tensors = create_test_tensors(
                params, table_dtype, output_dtype, false, false, "_reference");

        // Run reference kernel only
        status_t ref_status = EmbagTestUtils::run_reference_embag(tensors.table,
                tensors.indices, tensors.offsets, tensors.output, params);

        if (EmbagTestUtils::is_embag_reference_supported(
                    table_dtype, output_dtype, params.algo)) {
            EXPECT_EQ(ref_status, status_t::success)
                    << "Reference kernel must succeed for supported dtype/algo "
                       "combinations";
        } else {
    ...
    void run_accuracy_test(const EmbagParamsAI &params) {
        ...
        auto tensors = create_test_tensors(
                params, table_dtype, output_dtype, false, true, "_accuracy");

        AITestUtils::debug_print(
                "[AI_EMBAG_DEBUG] About to run ZenDNNL kernel...");
        bool kernel_supported = EmbagTestUtils::is_embag_kernel_supported(
                table_dtype, output_dtype, params.algo);
        ...
        if (EmbagTestUtils::is_embag_reference_supported(
            ...
            if (test_status == status_t::success) {
                ...
                AITestUtils::debug_print(
                        "[AI_EMBAG_DEBUG] About to run reference "
                        "implementation...");
                ...
                AITestUtils::debug_print(
                        "[AI_EMBAG_DEBUG] Reference implementation finished.");
                ...
                if (ref_status == status_t::success) {
                    ...
                    bool comparison_result
                            = EmbagTestUtils::compare_embag_tensors(
    ...
    void run_boundary_test(const EmbagParamsAI &params) {
        ...
        auto tensors = create_test_tensors(
                params, table_dtype, output_dtype, true, false, "_boundary");
   ...
   */
    void run_edge_case_test(const EmbagParamsAI &params) {
        ...
        auto tensors = create_test_tensors(
                params, table_dtype, output_dtype, false, false, "_edge");
   ...
   */
    ...
    void run_group_embag_test(const EmbagParamsAI &params) {
        ...
        ASSERT_GT(params.group_size, 0) << "Group size must be greater than 0";
        ...
        for (uint64_t i = 0; i < params.group_size; ++i) {
            ...
            auto tensors = create_test_tensors(
                    params, table_dtype, output_dtype, false, false, suffix);
            ...
            if (params.use_offsets) {
                ...
                embag_params_obj.include_last_offset
                        = params.include_last_offset;
            }

            embag_params_obj.is_weights = false;
            embag_params_obj.padding_idx
                    = params.use_padding_idx ? params.padding_idx : -1;
        ...
        AITestUtils::debug_print(
                "[AI_EMBAG_DEBUG] Running group_embedding_bag_direct with "
        ...
        AITestUtils::debug_print(
                "[AI_EMBAG_DEBUG] group_embedding_bag_direct finished.");

        if (params.expect_success) {
            EXPECT_EQ(status, status_t::success)
                    << "Group embedding bag test failed when success was "
                       "expected for "
                    << params.test_name;
        } else {
            EXPECT_NE(status, status_t::success)
                    << "Group embedding bag test succeeded when failure was "
                       "expected for "
                    << params.test_name;
        }

        std::cout << "[AI_EMBAG_GROUP] " << params.test_name << " with "
                  << params.group_size << " tables completed with status: "
...
private:
    ...
    status_t run_invalid_embag_test(const EmbagParamsAI &params) {
        try {
            auto table_dtype
                    = EmbagTestUtils::get_table_dtype(params.data_types);
            auto output_dtype
                    = EmbagTestUtils::get_output_dtype(params.data_types);

            // Try to create tensors with invalid dimensions
            auto tensors = create_test_tensors(params, table_dtype,
                    output_dtype, false, false, "_invalid");
    ...
    status_t run_embag_test(tensor_t &table, tensor_t &indices,
        ...
        try {
            ...
            if (is_lowoha_mode_enabled()) {
                ...
                AITestUtils::debug_print(
                        "[AI_EMBAG_DEBUG] Running LOWOHA "
                        "embedding_bag_direct...");

                // Validate tensors
                if (!table.check() || !indices.check() || !output.check()) {
                    std::cout << "[AI_EMBAG_TEST] LOWOHA: Invalid tensor state "
                                 "for "
                ...
                if (params.use_offsets && !offsets.check()) {
                    std::cout << "[AI_EMBAG_TEST] LOWOHA: Invalid offsets "
                                 "tensor for "
                              << params.test_name << std::endl;
                    return status_t::failure;
                }
                ...
                void *offsets_data = params.use_offsets
                        ? offsets.get_raw_handle_unsafe()
                        : nullptr;
                void *output_data = output.get_raw_handle_unsafe();

                if (!table_data || !indices_data || !output_data) {
                    std::cout
                            << "[AI_EMBAG_TEST] LOWOHA: Null data pointer for "
                            << params.test_name << std::endl;
                    return status_t::failure;
                }
                ...
                embag_params_obj.is_weights
                        = false; // AI tests don't use per-sample weights
                embag_params_obj.padding_idx
                        = params.use_padding_idx ? params.padding_idx : -1;
                ...
                if (status != status_t::success) {
                    std::cout << "[AI_EMBAG_TEST] LOWOHA embedding API failed "
                                 "for "
                              << params.test_name << std::endl;
                }
                return status;
                ...
                AITestUtils::debug_print(
                        "[AI_EMBAG_DEBUG] Running primitive embag operator...");
                ...
                if (params.use_padding_idx) {
                    embag_context = embag_context.set_padding_index(
                            params.padding_idx);
                }
                ...
                if (!embag_context.check()) {
                    std::cout << "[AI_EMBAG_TEST] Context creation failed for "
                              << params.test_name << std::endl;
                    return status_t::failure;
                }
                ...
                if (embag_operator.is_bad_object()) {
                    std::cout << "[AI_EMBAG_TEST] Operator creation failed for "
                              << params.test_name << std::endl;
                    return status_t::failure;
                }
                ...
                if (params.use_offsets) {
                    embag_operator
                            = embag_operator.set_input("offsets", offsets);
                }
            ...
            std::cout << "[AI_EMBAG_TEST] Unknown exception in "
                      << params.test_name << std::endl;
            return status_t::failure;
        }
    }
    ...
    // -----------------------------------------------------------------------------
    bool validate_output_tensor(
            const tensor_t &output, const EmbagParamsAI &params) {
        // Check dimensions
        ...
        if (output.get_data_type() != expected_dtype) {
            std::cout << "[AI_EMBAG_BOUNDARY_ERROR] Output tensor data type "
                         "mismatch"
                      << std::endl;
            return false;
        }
        ...
        if (output.get_nelem() > 0 && params.embedding_dim > 2) {
            ...
            if (!has_non_zero) {
                std::cout << "[AI_EMBAG_BOUNDARY_ERROR] Output is all zeros"
                          << std::endl;
                return false;
            }
        }
    ...
    bool validate_boundary_output(
            const tensor_t &output, const EmbagParamsAI &params) {
        if (!validate_output_tensor(output, params)) { return false; }
        ...
        if (total_elements <= 1000 && params.embedding_dim > 2
            ...
            std::cout << "[AI_EMBAG_BOUNDARY_ERROR] Small boundary case "
...
TEST_P(TestEmbagAI, ComprehensiveEmbagTest) {
    ...
    if (!EmbagTestUtils::is_valid_embag_data_type_combination(
                params.data_types)) {
...
INSTANTIATE_TEST_SUITE_P(AITests, TestEmbagAI,
        ::testing::ValuesIn(get_test_suite_for_mode<EmbagParamsAI,
                EmbagParameterGenerator>()),
