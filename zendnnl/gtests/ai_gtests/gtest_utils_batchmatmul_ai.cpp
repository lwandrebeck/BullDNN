...
bool BatchMatmulTestUtils::validate_batch_dimensions(
...
bool BatchMatmulTestUtils::is_valid_broadcast_config(
        bool broadcast_weights, bool broadcast_input) {
...
std::vector<uint64_t> BatchMatmulTestUtils::get_input_dims(
        uint64_t batch_size, uint64_t m, uint64_t k, bool broadcast_input) {
    if (broadcast_input) {
...
std::vector<uint64_t> BatchMatmulTestUtils::get_weight_dims(
        uint64_t batch_size, uint64_t k, uint64_t n, bool broadcast_weights) {
...
std::vector<uint64_t> BatchMatmulTestUtils::get_output_dims(
...
status_t BatchMatmulTestUtils::run_reference_batchmatmul(tensor_t &input,
        tensor_t &weights, tensor_t &bias, tensor_t &output,
    ...
    try {
        ...
        std::cerr << "[AI_BATCH_REF] Unknown exception in "
                     "run_reference_batchmatmul"
                  << std::endl;
        return status_t::failure;
    }
}
...
BatchMatmulParameterGenerator::get_fixed_dimensions_for_minimal_tests() {
    return {// Core shapes - essential for functional coverage
            {2, 4, 4, 4, "tiny_square"}, {2, 3, 5, 4, "tiny_rectangular"},
            {4, 32, 32, 32, "small_square"}, {8, 64, 64, 64, "medium_square"},
            {4, 128, 128, 128, "large_square"},
            {8, 64, 32, 48, "rectangular_1"}, {4, 32, 64, 48, "rectangular_2"},
            {2, 42, 42, 42, "non_power_of_2"}, {4, 512, 4, 4, "skinny_tall"},
            {4, 4, 512, 4, "skinny_wide"}, {4, 4, 4, 512, "skinny_deep"},
...
BatchMatmulParameterGenerator::generate_random_params_for_accuracy_subcategory(
    ...
    auto generate_dims
            = [&](uint64_t min_m, uint64_t max_m, uint64_t min_n,
                      uint64_t max_n, uint64_t min_k, uint64_t max_k,
                      BatchSizeDimensions min_batch,
                      BatchSizeDimensions max_batch, bool square = false) {
        ...
        if (broadcast_dist(batch_param_rng) == 0) { broadcast_weights = true; }
    ...
    if (category == "tiny_square") {
        generate_dims(static_cast<uint64_t>(MatrixDimensions::TINY_MIN),
                ...
                static_cast<uint64_t>(MatrixDimensions::TINY_MIN),
                static_cast<uint64_t>(MatrixDimensions::TINY_MAX),
                BatchSizeDimensions::TINY_MIN, BatchSizeDimensions::TINY_MAX,
                true);
        ...
        generate_dims(static_cast<uint64_t>(MatrixDimensions::SMALL_MIN),
                ...
                static_cast<uint64_t>(MatrixDimensions::SMALL_MIN),
                static_cast<uint64_t>(MatrixDimensions::SMALL_MAX),
                BatchSizeDimensions::SMALL_MIN, BatchSizeDimensions::SMALL_MAX,
                true);
        ...
        generate_dims(static_cast<uint64_t>(MatrixDimensions::MEDIUM_MIN),
                ...
                static_cast<uint64_t>(MatrixDimensions::MEDIUM_MIN),
                static_cast<uint64_t>(MatrixDimensions::MEDIUM_MAX),
                BatchSizeDimensions::MEDIUM_MIN,
                BatchSizeDimensions::MEDIUM_MAX, true);
        ...
        generate_dims(static_cast<uint64_t>(MatrixDimensions::LARGE_MIN),
                ...
                static_cast<uint64_t>(MatrixDimensions::LARGE_MIN),
                static_cast<uint64_t>(MatrixDimensions::LARGE_MAX),
                BatchSizeDimensions::LARGE_MIN, BatchSizeDimensions::LARGE_MAX,
                true);
        ...
        generate_dims(static_cast<uint64_t>(MatrixDimensions::RECT_MIN),
                ...
                static_cast<uint64_t>(MatrixDimensions::RECT_MAX),
                BatchSizeDimensions::MEDIUM_MIN,
                BatchSizeDimensions::MEDIUM_MAX, false);
        ...
        if (shape_type == 0) { // tall
            generate_dims(
                    ...
                    static_cast<uint64_t>(MatrixDimensions::SKINNY_MIN),
                    static_cast<uint64_t>(MatrixDimensions::SKINNY_MAX_SMALL),
                    BatchSizeDimensions::SKINNY_MIN,
                    BatchSizeDimensions::SKINNY_MAX, false);
        ...
        generate_dims(static_cast<uint64_t>(MatrixDimensions::TINY_MIN),
                ...
                static_cast<uint64_t>(MatrixDimensions::TINY_MIN),
                static_cast<uint64_t>(MatrixDimensions::TINY_MIN),
                BatchSizeDimensions::MIN_BATCH, BatchSizeDimensions::MIN_BATCH,
                false);
        ...
        generate_dims(AI_MAX_DIM / 4, AI_MAX_DIM / 2, AI_MAX_DIM / 4,
                AI_MAX_DIM / 2, AI_MAX_DIM / 4, AI_MAX_DIM / 2,
        ...
        generate_dims(static_cast<uint64_t>(MatrixDimensions::TINY_MIN),
                ...
                static_cast<uint64_t>(MatrixDimensions::TINY_MIN),
                static_cast<uint64_t>(MatrixDimensions::TINY_MAX),
                BatchSizeDimensions::XL_MIN, BatchSizeDimensions::XL_MAX,
                false);
        ...
        generate_dims(static_cast<uint64_t>(MatrixDimensions::TINY_MIN),
                ...
                static_cast<uint64_t>(MatrixDimensions::TINY_MIN),
                static_cast<uint64_t>(MatrixDimensions::TINY_MAX),
                BatchSizeDimensions::XXL_MIN, BatchSizeDimensions::XXL_MAX,
                false);
    ...
    return create_param(batch_size, m, n, k, data_combo, TestCategory::ACCURACY,
            post_op_config, broadcast_weights, broadcast_input, expect_success,
            "accuracy");
}
...
BatchMatmulParameterGenerator::generate_coverage_test_suite() {
    ...
    for (auto data_combo : ParameterGenerator::supported_combinations) {
        if (AITestUtils::is_aocl_kernel_supported(
            ...
            coverage_params.push_back(create_param(cov_batch, cov_m, cov_n,
                    cov_k, data_combo, TestCategory::ACCURACY, no_postop, false,
                    false, true, "coverage_bmm_dtype"));
        }
    }
    ...
    std::vector<std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, std::string>>
            batch_sizes
            = {{1, 32, 32, 32, "single_batch"}, {2, 32, 32, 32, "small_batch"},
    ...
    coverage_params.push_back(create_param(4, 32, 32, 32,
            ...
            true, false, true, "coverage_bmm_broadcast_weight"));
    // 2D input (input shared across batch)
    coverage_params.push_back(create_param(4, 32, 32, 32,
            DataTypeCombination::F32_F32_F32, TestCategory::ACCURACY, no_postop,
            false, true, true, "coverage_bmm_broadcast_input"));

    // 4. Boundary tests
    std::vector<std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, std::string>>
            boundary_dims
            = {{1, 1, 1, 1, "minimal"}, {1, 32, 32, 1, "single_k"},
                    {8, 8, 8, 8, "simd8"}, {4, 16, 16, 16, "avx16"}};
    for (const auto &[batch, m, n, k, desc] : boundary_dims) {
        coverage_params.push_back(
                create_param(batch, m, n, k, DataTypeCombination::F32_F32_F32,
                        TestCategory::BOUNDARY, no_postop, false, false, true,
                        "coverage_bmm_boundary_" + desc));
    }

    // 5. Edge case tests
    std::vector<std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, std::string>>
            edge_dims = {{2, 1, 128, 128, "tall"}, {2, 128, 1, 128, "wide"},
                    {2, 128, 128, 1, "flat_k"}};
    ...
    coverage_params.push_back(create_param(0, 32, 32, 32,
            ...
            false, false, false, "coverage_bmm_invalid_zero_batch"));
    // Zero dimension
    coverage_params.push_back(create_param(4, 0, 32, 32,
            DataTypeCombination::F32_F32_F32, TestCategory::INVALID, no_postop,
            false, false, false, "coverage_bmm_invalid_zero_m"));
    // Unsupported dtype
    coverage_params.push_back(create_param(4, 32, 32, 32,
            DataTypeCombination::S4_S4_S4, TestCategory::INVALID, no_postop,
            false, false, false, "coverage_bmm_invalid_dtype"));
            ...
            for (const auto &[batch, m, n, k, desc] : fixed_dims) {
                if (AITestUtils::is_aocl_kernel_supported(
                    ...
                    params.push_back(create_param(batch, m, n, k, data_combo,
                            TestCategory::ACCURACY, post_op_config, false,
                            false, true, "minimal"));
                }
            }
        }
...
void BatchMatmulParameterGenerator::add_accuracy_params(
    ...
    auto post_op_configs = AITestUtils::get_all_post_op_configs();
    const std::vector<std::string> categories = {"tiny_square",
            "tiny_rectangular", "small_square", "medium_square", "large_square",
            "rectangular", "skinny", "min", "max", "xl_batch_tiny",
            "xl_batch_small", "xxl_batch_tiny", "xxl_batch_small"};
    ...
    auto get_max_cases = [](const std::string &category) -> int {
        ...
        if (category == "small_square") { return MaxTestCases::SMALL_MATRIX; }
        ...
        if (category == "skinny") { return MaxTestCases::SKINNY_MATRIX; }
        if (category == "min") { return MaxTestCases::MIN; }
        if (category == "max") { return MaxTestCases::MAX; }
...
void BatchMatmulParameterGenerator::add_boundary_params(
    ...
    std::vector<std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, std::string>>
            boundary_dims
            = {{1, 1, 1, 1, "minimal_all"}, {1, 1, 32, 32, "minimal_batch"},
                    {32, 1, 32, 32, "minimal_m"}, {32, 32, 1, 32, "minimal_n"},
                    {32, 32, 32, 1, "minimal_k"}, {8, 8, 8, 8, "simd_boundary"},
        ...
        for (const auto &post_op_config : post_op_configs) {
            for (const auto &[batch, m, n, k, desc] : boundary_dims) {
                if (AITestUtils::is_aocl_kernel_supported(
                            ...
                            post_op_config.post_ops)) {
                    params.push_back(create_param(batch, m, n, k, data_combo,
                            TestCategory::BOUNDARY, post_op_config, false,
                            false, true, "boundary"));
                }
            }
        }
        ...
        for (const auto &post_op_config : post_op_configs) {
            for (const auto &tup : edge_dims) {
                ...
                if (AITestUtils::is_aocl_kernel_supported(
                            ...
                            post_op_config.post_ops)) {
                    params.push_back(create_param(batch, m, n, k, data_combo,
                            TestCategory::EDGE_CASE, post_op_config, false,
                            false, true, "edge_case"));
                }
            }
        }
    }
...
void BatchMatmulParameterGenerator::add_invalid_params(
        ...
        for (auto post_op_config : post_op_configs) {
            if (AITestUtils::is_aocl_kernel_supported(
                ...
                params.push_back(create_param(0, 32, 32, 32, data_combo,
                        TestCategory::INVALID, post_op_config, false, false,
                        false, "invalid"));
                // Invalid dimensions
                ...
                params.push_back(create_param(AI_MAX_DIM + 1, 32, 32, 32,
    ...
    params.push_back(create_param(4, 8, 8, 8, DataTypeCombination::F32_F32_F32,
            TestCategory::INVALID, post_op_configs[0], true, true, false,
            "invalid"));
}
        ...
        for (const auto &post_op_config : post_op_configs) {
            if (AITestUtils::is_aocl_kernel_supported(
                ...
                params.push_back(create_param(4, 32, 32, 32, data_combo,
                        TestCategory::ACCURACY, post_op_config, true, false,
                        true, "accuracy"));
                // Broadcast input (3D weights, 2D input)
                params.push_back(create_param(4, 32, 32, 32, data_combo,
                        TestCategory::ACCURACY, post_op_config, false, true,
                        true, "accuracy"));
            }
        }
    }
...
void BatchMatmulParameterGenerator::generate_reference_kernel_exhaustive_params(
    ...
    std::vector<std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, std::string>>
            ref_dims
            = {{2, 4, 4, 4, "tiny_square"}, {2, 4, 3, 2, "tiny_rectangular"},
...
BatchMatmulParamsAI BatchMatmulParameterGenerator::create_param(
        uint64_t batch_size, uint64_t m, uint64_t n, uint64_t k,
        DataTypeCombination combo, TestCategory category,
        const PostOpConfig &post_op_config, bool broadcast_weights,
        bool broadcast_input, bool expect_success,
    ...
    if (broadcast_weights) { broadcast_str += "_bw"; }
    if (broadcast_input) { broadcast_str += "_bi"; }

    std::string suite_prefix = suite_name.empty() ? "" : suite_name + "_";
    param.test_name = suite_prefix + "b" + std::to_string(batch_size) + "_m"
            + std::to_string(m) + "_n" + std::to_string(n) + "_k"
            + std::to_string(k) + "_in_" + input_dtype_str + "_wt_"
            + weight_dtype_str + "_out_" + output_dtype_str + "_"
            + post_op_config.config_name + broadcast_str + "_"
