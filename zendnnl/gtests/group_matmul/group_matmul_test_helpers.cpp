...
void PrintTo(const GroupQuantMatmulType &value, ::std::ostream *os) {
    *os << "m=" << value.matmul_m << ", k=" << value.matmul_k
        << ", n=" << value.matmul_n << ", algo=" << algoToStr(value.algo)
...
status_t group_matmul_kernel_test(std::vector<tensor_t> &inputs,
        ...
        std::vector<tensor_t> &outputs, matmul_algo_t algo, float alpha,
        float beta, const group_matmul_moe_postop_params *moe_postop,
    ...
    try {
        ...
        std::vector<const void *> srcs(num_ops), wts(num_ops),
                bias_ptrs(num_ops);
        ...
        for (size_t i = 0; i < num_ops; ++i) {
            if (!inputs[i].check() || !weights[i].check()
                    || !outputs[i].check()) {
                log_error("group_matmul_kernel_test: invalid tensor at expert ",
                        i);
                return status_t::failure;
            }
            ...
            const bool is_int4_wei = (src_dt == data_type_t::bf16
                    && (wei_dt == data_type_t::s4
                            || wei_dt == data_type_t::u4));
            const bool is_w4a8 = (src_dt == data_type_t::bf16
                    && wei_dt == data_type_t::s4 && inputs[i].is_quantized());
            ...
            bias_ptrs[i] = (skip_bias || !biases[i].check())
                    ...
                    : static_cast<const void *>(
                              biases[i].get_raw_handle_unsafe());
            dsts[i] = outputs[i].get_raw_handle_unsafe();
        }

        status_t status = group_matmul_direct(layouts, transAs, transBs, Ms, Ns,
                Ks, alphas, srcs, ldas_v, wts, ldbs_v, bias_ptrs, betas, dsts,
                ldcs_v, is_wt_c, params_v, moe_postop, gated_act);
