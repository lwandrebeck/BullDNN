...
TEST_P(TestReorder, FP32_DYN_QUANT) {
    ...
    if (quant_status != status_t::success) {
        log_error("Dynamic quantization (F32 -> ", is_symmetric ? "S8" : "U8",
                ") failed");
        EXPECT_TRUE(false);
        return;
    }
...
TEST_P(TestReorder, BF16_DYN_QUANT) {
    ...
    status_t quant_status = lowoha_reorder_kernel_test(src_tensor, quant_tensor,
            scale_tensor, zp_tensor, lowoha_params, /*dynamic_quant=*/true);
    if (quant_status != status_t::success) {
        log_error("Dynamic quantization (BF16 -> ", is_symmetric ? "S8" : "U8",
                ") failed");
        EXPECT_TRUE(false);
        return;
    }
...
TEST_P(TestReorder, F16_DYN_QUANT) {
    ...
    if (quant_status != status_t::success) {
        log_error("Dynamic quantization (F16 -> ", is_symmetric ? "S8" : "U8",
                ") failed");
        EXPECT_TRUE(false);
        return;
    }
