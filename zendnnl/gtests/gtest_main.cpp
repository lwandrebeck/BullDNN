...
int main(int argc, char **argv) {
    try {
        ...
        dtype_arr = {data_type_t::f32, data_type_t::bf16, data_type_t::s8,
                data_type_t::u8, data_type_t::f16};
        ...
        if (!cmd_input_file.empty() && cmd_operator.empty()) {
            commonlog_error(
                    "--input_file requires --op "
                    "(matmul, reorder, embeddingbag, embedding, "
                    "normalization).\n");
            return 1;
        }
        if (!cmd_operator.empty()
                && k_input_file_ops.find(cmd_operator)
                        == k_input_file_ops.end()) {
        ...
        if (!cmd_input_file.empty() && cmd_operator == "matmul" && ndims != 2
                && ndims != 3) {
            commonlog_error(
                    "--op matmul with --input_file requires --ndims 2 or 3 "
                    "(got "
                    + std::to_string(ndims) + ").\n");
            return 1;
        }
        if (!cmd_input_file.empty() && cmd_operator == "reorder") {
            if (cmd_lowoha.empty()) {
                commonlog_error(
                        "Reorder input file requires --lowoha true or --lowoha "
                        "false "
        ...
        if (input_file_mode && cmd_operator == "matmul" && ndims == 2) {
            ...
            if (matmul_inputs.empty()) {
                commonlog_error(
                        "No valid test inputs parsed from --input_file \""
                        + cmd_input_file
            ...
            for (uint32_t i = 0; i < test_num; ++i) {
                matmul_test[i]
                        = MatmulType(cli_params.matmul_input, i, test_num);
            }
        }
        if (!input_file_mode) {
        ...
        if (input_file_mode && cmd_operator == "matmul" && ndims == 3) {
            ...
            for (const auto &input : matmul_inputs) {
                try {
                    for (uint32_t i = 0; i < test_num; ++i) {
                        batchmatmul_test.push_back(
                                BatchMatmulType(input, i, test_num));
                    }
                } catch (const std::exception &e) { commonlog_error(e.what()); }
            }
            ...
            for (uint32_t i = 0; i < test_num; ++i) {
                batchmatmul_test[i]
                        = BatchMatmulType(cli_params.matmul_input, i, test_num);
            }
        }
        if (input_file_mode && cmd_operator == "reorder") {
            std::cout << "Using input file: " << cmd_input_file
                      << " (lowoha=" << cmd_lowoha << ")" << std::endl;
            const bool is_lowoha_reorder
                    = *parse_bool_field(cmd_lowoha, "lowoha");
            auto reorder_inputs
                    = read_reorder_inputs(cmd_input_file, is_lowoha_reorder);
            ...
            for (uint32_t i = 0; i < test_num; ++i) {
                reorder_test.push_back(
                        ReorderType(cli_params.reorder_input, i, test_num));
            }
        }
        ...
        if (input_file_mode && cmd_operator == "embedding") {
            ...
            for (uint32_t i = 0; i < test_num; ++i) {
                embedding_test.push_back(
                        EmbeddingType(cli_params.embedding_input));
            }
        }
        ...
        if (input_file_mode && cmd_operator == "normalization") {
            ...
            for (uint32_t i = 0; i < test_num; ++i) {
                normalization_test.push_back(
                        NormalizationType(cli_params.normalization_input));
            }
        }
