...
public:
    explicit scoped_thread_algo(eb_thread_algo_t algo) {
        ...
        setenv(kEnvName, std::to_string(static_cast<int32_t>(algo)).c_str(),
                /*overwrite=*/1);
    }
    ~scoped_thread_algo() {
        if (had_prev_) {
...
GroupEmbagType::GroupEmbagType(uint32_t test_index, uint32_t total_tests) {
    ...
    algo = cli_params.embag_input.embag_algo
            ...
            : static_cast<embag_algo_t>(
                      1 + std::rand() % 3); // sum / mean / max

    padding_index = cli_params.embag_input.embedding_input.padding_index
            ? *cli_params.embag_input.embedding_input.padding_index
            : -1;
    ...
    is_weights = cli_params.embag_input.embedding_input.is_weights
            ? *cli_params.embag_input.embedding_input.is_weights
            : (std::rand() % 2);

    if (cli_params.embag_input.embedding_input.indices_dtype
            && (*cli_params.embag_input.embedding_input.indices_dtype
                            == data_type_t::s32
                    || *cli_params.embag_input.embedding_input.indices_dtype
                            == data_type_t::s64)) {
        ...
        indices_dtype
                = (std::rand() % 2 == 0) ? data_type_t::s32 : data_type_t::s64;
    }
...
void PrintTo(const GroupEmbagType &v, ::std::ostream *os) {
    *os << "num_embeddings=" << v.num_embeddings
        ...
        << ", num_indices=" << v.num_indices << ", num_bags=" << v.num_bags
        ...
        << ", num_threads=" << v.num_threads << ", seed=" << seed;
}
...
GroupTensors build_group_embag_tensors(tensor_factory_t &factory,
    ...
    for (size_t i = 0; i < group_size; ++i) {
        ...
        g.offsets.push_back(factory.random_offsets_tensor({offsets_size},
        ...
        g.weights.push_back(is_weights
                        ...
                        : tensor_t {});
        g.outputs.push_back(
                factory.zero_tensor({num_bags, embedding_dim}, output_dtype));
        g.outputs_ref.push_back(
                factory.zero_tensor({num_bags, embedding_dim}, output_dtype));
    }
    return g;
}
...
bool compare_group_outputs(GroupTensors &g, uint64_t output_rows,
    ...
    for (size_t i = 0; i < g.outputs.size(); ++i) {
        bool table_ok = true;
        compare_tensor_2D(g.outputs[i], g.outputs_ref[i], output_rows,
                embedding_dim, tol, table_ok);
        if (!table_ok) {
...
status_t group_embag_kernel_test(std::vector<tensor_t> &tables,
    ...
    try {
        ...
        if (num_tables == 0 || indices.size() != num_tables
                || offsets.size() != num_tables || weights.size() != num_tables
                || outputs.size() != num_tables || algos.size() != num_tables
        ...
        for (size_t i = 0; i < num_tables; ++i) {
            if (!tables[i].check() || !indices[i].check()
                    || !outputs[i].check()) {
                log_error(
                        "group_embag_kernel_test: invalid tensor at table ", i);
                return status_t::failure;
            }

            const bool lookup_mode = (algos[i] == embag_algo_t::none);
            if (!lookup_mode && !offsets[i].check()) {
                log_error("group_embag_kernel_test: offsets required at table ",
                        i, " for reduction algo");
                return status_t::failure;
            }
            ...
            weights_ptrs[i] = weights[i].check()
                    ? static_cast<const float *>(
                              weights[i].get_raw_handle_unsafe())
                    : nullptr;
            ...
            p.num_bags = lookup_mode
                    ? 0
                    : (include_last_offsets[i] ? offsets[i].get_size(0) - 1
        ...
        return group_embedding_bag_direct(table_ptrs, indices_ptrs,
                offsets_ptrs, weights_ptrs, output_ptrs, params_v);
...
status_t group_embag_forced_ref_kernel_test(std::vector<tensor_t> &tables,
    ...
    for (size_t i = 0; i < num_tables; ++i) {
        ...
        status_t st = lookup_mode
                ? embedding_kernel_test(tables[i], indices[i], weights[i],
                          outputs[i], padding_idxs[i], is_weights,
                          fp16_scale_bias[i], embag_kernel_t::reference, true)
                : embag_kernel_test(tables[i], indices[i], offsets[i],
                          weights[i], outputs[i], algos[i], padding_idxs[i],
