...
protected:
    void SetUp() override {
        ...
        log_info("GroupEmbag test: num_embeddings=", num_embeddings,
                " embedding_dim=", embedding_dim, " num_indices=", num_indices,
                " num_bags=", num_bags, " algo=", static_cast<int>(algo),
    ...
    void run_float_group_test(
            data_type_t table_dt, data_type_t output_dt, float tol) {
        GroupTensors g = build_group_embag_tensors(tensor_factory, group_size,
                num_embeddings, embedding_dim, num_indices, num_bags, algo,
                padding_index, include_last_offset, is_weights, fp16_scale_bias,
                table_dt, output_dt, indices_dtype, offsets_dtype);

        status_t status = group_embag_kernel_test(g.tables, g.indices,
                g.offsets, g.weights, g.outputs, g.algos, g.padding_idxs,
                g.include_last_offsets, g.fp16_scale_bias, thread_algo);

        if (status == status_t::isa_unsupported) {
            GTEST_SKIP() << "F16 not supported: requires F16-capable ISA";
        }

        status_t ref_status = group_embag_forced_ref_kernel_test(g.tables,
                g.indices, g.offsets, g.weights, g.outputs_ref, g.algos,
                g.padding_idxs, g.include_last_offsets, g.fp16_scale_bias);

        bool ok = (status == status_t::success
                && ref_status == status_t::success);
        if (ok) { ok = compare_group_outputs(g, num_bags, embedding_dim, tol); }
        EXPECT_TRUE(ok);
    }
    ...
    // status.
    void run_quant_group_test(
            data_type_t table_dt, data_type_t output_dt, float tol) {
        GroupTensors g = build_group_embag_quant_tensors(tensor_factory,
                group_size, num_embeddings, embedding_dim, num_indices,
                num_bags, algo, padding_index, include_last_offset, is_weights,
                fp16_scale_bias, table_dt, output_dt, indices_dtype,
                offsets_dtype);
...
INSTANTIATE_TEST_SUITE_P(
        GroupEmbag, TestGroupEmbag, ::testing::ValuesIn(group_embag_test));
